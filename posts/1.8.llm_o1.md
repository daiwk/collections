...
+ 去[https://huggingface.co/deepseek-ai/DeepSeek-R1/tree/main](https://huggingface.co/deepseek-ai/DeepSeek-R1/tree/main)把各个json文件下载下来，还有configuration_deepseek.py和modeling_deepseek.py也要下载，把json里的torch_dtype里的bfloat16改成float16
+ 去[https://huggingface.co/unsloth/DeepSeek-R1-GGUF/tree/main](https://huggingface.co/unsloth/DeepSeek-R1-GGUF/tree/main)把config.json下载下来

上面的那些文件都放到/xxx/目录下，注意，目前只能用Q2 Q4那种量化，1.58bit那种动态量化不支持

```shell
cd /xxx/
# merge成一个gguf
llama.cpp/llama-gguf-split --merge ./DeepSeek-R1-xx/DeepSeek-R1-xx-00001-of-00003.gguf ./merge.gguf
```
...
```shell
VLLM_WORKER_MULTIPROC_METHOD=spawn python3 vllm_deepseek.py 
```
