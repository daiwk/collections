...
+ R1-zero如何成为reasoner：
  ...
  + GRPO可并行，可加速
    + 正常rl中：
      ...
      + policy model：base llm，需要训练
        + policy：一条llm的回复；
        + action：llm生成的每个token
      + value function：reward model，对一条完整的回复（一个policy）打分。可以是一个model，也可以是人（rlhf）
      + value model：类似reward model，需要训练
      ...
      + 梯度更新：更新policy模型
      + 缺点如下：
        + 需要ref model再算一遍，有计算量
        + 需要拆解到每个token，又有计算量
    + GRPO：
      + policy model/action/ref model都和正常rl一样
      + 不需要value model
      + 一下子执行更多的policy，产出的结果一起打分，算出标准差，把每个policy和标准差的差距平均到每个token上，平均完后再用KL散度计算和ref model的差距

       
![](../assets/ppo-grpo.png)

![](../assets/ppo-grpo2.png)
