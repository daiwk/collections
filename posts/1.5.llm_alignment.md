...
![rlhf-sft](<.gitbook/assets/rlhf-sft (1).png>)
...
![rlhf-rm](<.gitbook/assets/rlhf-rm (1).png>)
...
![rlhf-rl](<.gitbook/assets/rlhf-rl (1).png>)
...
![rlhf-ppo-flows-orig](<.gitbook/assets/rlhf-ppo-flows-orig (1).png>)
...
![rlhf-workflow](<.gitbook/assets/rlhf-workflow (1).jpeg>)
...
* New Policy Sampling：
  * 输入ref\_model、actor、critic，从新的策略中采样概率等信息，
...
![rlhf-dot](<.gitbook/assets/rlhf-dot (2).jpg>)
...
critic是专门用来预测actor轨迹**每一步价值**的网络(**这个是和reward model的核心区别，reward model只给出整个句子的reward，且不能训练，critic是每一步都能给一个值，且能训练**)，actor上加几个线性层能够给每个token预测一个值。任务是估计状态的价值函数，也就是预测从当前状态开始，通过遵循某个策略，期望能得到的总回报。
...
[https://huggingface.co/docs/trl/dataset\_formats#which-dataset-type-to-use](https://huggingface.co/docs/trl/dataset_formats#which-dataset-type-to-use)
...
![rlaif-rlhf](<.gitbook/assets/rlaif-rlhf (1).png>)

如上是rlaif和rlhf的对比。

![ai-gen-prefer-label](<.gitbook/assets/ai-gen-prefer-label (1).png>)
...
![d-rlaif](<.gitbook/assets/d-rlaif (1).png>)
...
[https://github.com/facebookresearch/RAM/tree/main/projects/self\_taught\_evaluator](https://github.com/facebookresearch/RAM/tree/main/projects/self_taught_evaluator)
...
![rft-grader](<.gitbook/assets/rft-grader (1).png>)
...
![](<.gitbook/assets/rm-patterns (1).png>)
...
第一篇应该是[MiniLLM: Knowledge Distillation of Large Language Models](https://arxiv.org/pdf/2306.08543v3)，里面对比了

![](<.gitbook/assets/minillm (1).png>)

之前的蒸馏都是用teacher采样出一个序列，然后student去学它，用forward KL；这篇提出的是用student采样序列，然后看teacher打分，student去学，用reverse KL(当然，后面可以看到别的变种不是用RKL，但从student采样这个是opd的核心)

简单理解就是：student采样出一个序列，比如输入是xxx，student输出了a，然后这个时候teacher也输入xxx，看最后那个softmax对a的打分是多少，让student去学；然后下一步，student输入xxxa，输出b，teacher也是输入xxxa，看b的打分

forward KL vs reverse KL

![](<.gitbook/assets/fkl-rkl (1).png>)

## 后训练的一些思考

参考：[xhs](https://www.xiaohongshu.com/explore/68ddbb12000000000300ccf3?app_platform=ios\&app_version=9.3.1\&share_from_user_hidden=true\&xsec_source=app_share\&type=normal\&xsec_token=CBS14H4N6Qj9203eE3W-PSZYAeS09fuF0w_vOwrIeRr3Y=\&author_share=1\&xhsshare=WeixinSession\&shareRedId=ODY2NUg4NE82NzUyOTgwNjY0OTc1STdO\&apptime=1759366426\&share_id=35b7932b0dc34529ba9116f3f0972a00)

* 对sft和rl的两种理解之一：模仿和泛化。目的上都是实现某种对齐，只不过rl会在奖励合适情况下向目标**更平滑过渡**，而不会像sft**阶跃式变化**。在这种目的下，很多做法会先sft冷启动再rl微调，两个阶段数据量比例在1:3到1:10左右。
* 对sft和rl的两种理解之二：激发推理。sft激发方式是关联不同数据的**同质化逻辑**，底层来源在训练数据的质量、数量、和多样性；而rl的激发方式是**比当前模型略强的奖励信号**，底层来源是模型参数分布和奖励最高值之间的恰当gap。
* 模型输出的熵很重要，尤其是高条件熵下输出高概率token，和低条件熵下输出低概率token这两种特殊cases，可能对激发模型内化推理能力有较大作用。
* 作为后训练中为数不多的可以修改网络结构的模型，奖励模型的训练比多数垂域dpo要更复杂，且依赖数据量更大，更有性价比的做法可能还是用rubric judge(可能是llm as a judge?)。
* 在对垂域任务设计奖励系统时，对几十B模型的奖励策略可能难以直接扩展到上百B模型。
* 高质量，多样化的合成数据是提升模型性能的必要非充分条件，这里的合成不仅有对话轮数粒度的合成，还有思考粒度、句子粒度的合成，也见过在思考链里插入一些个别token的合成方法。
...
special_tokens = [f"<SID_{i}>" for i in range(2000)]
...
### 只对special token的emb微调

```python
import torch

original_vocab_size = tokenizer.vocab_size - num_added

# 1) 先按你的逻辑：其他参数正常训练
for name, param in model.named_parameters():
    param.requires_grad = True

# 2) embedding / lm_head 也先整体打开
...
# 3) 构造 mask：只保留新增 token 的行梯度
def build_row_mask(weight, original_vocab_size):
    mask = torch.zeros(weight.size(0), dtype=torch.bool, device=weight.device)
    mask[original_vocab_size:] = True
    return mask

input_mask = build_row_mask(input_emb.weight, original_vocab_size)
...
input_emb.weight.register_hook(input_grad_hook)

# 4) lm_head 若与 embedding 不共享权重，则也单独处理
if output_emb is not None and output_emb.weight is not input_emb.weight:
    output_mask = build_row_mask(output_emb.weight, original_vocab_size)
    ...
    output_emb.weight.register_hook(output_grad_hook)
```
...
# 4) LoRA + 仅训练这些 token 的 embedding 行
