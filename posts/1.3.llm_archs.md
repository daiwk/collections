...
+ Attention通常是访存瓶颈（memory access bound），当增大batch size时，Attention的算力需求基本保持不变，因其已达到数据加载的上限
+ FFN则更多表现为计算瓶颈（compute bound）。当增大batch size时，FFN能够获得一定的性能提升，这一特点在MoE架构中尤为明显，
