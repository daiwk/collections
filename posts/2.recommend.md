...
[https://blog.csdn.net/qq\_22210253/article/details/85229988](https://blog.csdn.net/qq_22210253/article/details/85229988)

CrossEntropyLoss就是把Softmax–Log–NLLLoss合并成一步

![cnllloss](<.gitbook/assets/ce-nllloss (1).png>)
...
![swing](<.gitbook/assets/swing (1).png>)

### 内积、余弦和L2

![](<.gitbook/assets/L2-innerproduct-hnsw (1).png>)
...
![dssm\_2013cikm](<.gitbook/assets/dssm_2013cikm (1).png>)
...
![multiview-dnn](<.gitbook/assets/multiview-dnn (1).png>)
...
tf代码：[https://github.com/InsaneLife/dssm/blob/master/multi\_view\_dssm\_v3.py](https://github.com/InsaneLife/dssm/blob/master/multi_view_dssm_v3.py)
...
\==>对于负样本采样率（保留百分之m的负样本）是m，正样本采样率（保留百分之n的正样本）是n

$$
e^{-logit'}=\frac{1}{e^{logit'}}=1/odds=实际负样本数/实际正样本数=m/n * 负样本数/正样本数=m/n * e^{-logit}
$$

然后对于经典的召回采样纠偏([Sampling-Bias-Corrected Neural Modeling for Large Corpus Item Recommendations](https://storage.googleapis.com/gweb-research2023-media/pubtools/5716.pdf))来讲，就是$$logit' = logit - log(p)$$，就有如下公式，和上面是类似的

$$
exp(-logit')=exp(-logit+log(p))=p * exp(-logit)
$$

参考tf的weighted sigmoid：`weighted_cross_entropy_with_logits`: [https://www.tensorflow.org/api\_docs/python/tf/nn/weighted\_cross\_entropy\_with\_logits](https://www.tensorflow.org/api_docs/python/tf/nn/weighted_cross_entropy_with_logits)
...
![youtube-dnn-recsys-architecture](<.gitbook/assets/youtube-dnn-recsys-architecture (1).png>)
...
在init\_data函数中，给每个\_\_label\_\_xx编了个号，如：
...
然后read\_data的时候，y就用这个编号来表示（假装是时长）：

```python
y.append(label_dict[line[0]])
```

而使用的是nce\_loss(参考[https://daiwk.github.io/posts/knowledge-tf-usage.html#tfnnnceloss](https://daiwk.github.io/posts/knowledge-tf-usage.html#tfnnnceloss))：
...
[http://demo.clab.cs.cmu.edu/cdyer/nce\_notes.pdf](http://demo.clab.cs.cmu.edu/cdyer/nce_notes.pdf)
...
![ssl1](<.gitbook/assets/ssl-1 (1).png>)

![ssl1](<.gitbook/assets/ssl-2 (1).png>)
...
[自己下载了](https://github.com/daiwk/collections/blob/master/assets/Q-align.pdf)
...
![byol](<.gitbook/assets/byol (1).png>)
...
![](<.gitbook/assets/mind-comirec (1).png>)
...
Customer Shopping TrAjectory Representation Learning framework (C-STAR) PR-Graph（product relational graph）：$$\mathcal{G}=(\mathcal{T}, \mathcal{E}, \mathcal{V})$$
...
![c-star-algo](<.gitbook/assets/c-star-algo (1).png>)
...
[Stochastic Gradient Boosted Distributed Decision Trees](https://github.com/daiwk/collections/blob/master/assets/gbdt-Stochastic%20Gradient%20Boosted%20Distributed%20Decision%20Trees.pdf)
...
![](<.gitbook/assets/baoxuhuigui (1).png>)
...
delay feedback [https://zhuanlan.zhihu.com/p/555950153](https://zhuanlan.zhihu.com/p/555950153)
...
cgc参考paddle代码：[cgc\_demo.py](https://github.com/daiwk/collections/blob/master/assets/cgc_dir/cgc_demo.py)
