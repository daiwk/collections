...
![llm-rec-where](<.gitbook/assets/llm-rec-where (1).png>)

![llm-rec-how](<.gitbook/assets/llm-rec-how (1).png>)
...
![s3-rec](<.gitbook/assets/s3-rec (1).png>)

这些方法都是学习item的向量，然后用MIPS去ANN，而TIGER(Transformer Index for GEnerative Recommenders)则是生成式地直接预测item的语义id

![tiger](<.gitbook/assets/tiger (1).png>)

![tiger-quant](<.gitbook/assets/tiger-quant (1).png>)

![tiger-personalized](<.gitbook/assets/tiger-personalized (1).jpg>)

rq-vae的介绍：

![rq-vae](<.gitbook/assets/rq-vae (1).png>)
...
![](<.gitbook/assets/sid-no-seq (1).png>)
...
![](<.gitbook/assets/sid-has-seq (1).png>)
...
![nce-metric](<.gitbook/assets/nce-metric (1).png>)

可以发现，对于一个预训练好的模型来说，随着时间的变化，NCE的变化是很剧烈的，即数据分布是会漂移的，并不像nlp/cv一样有ground truth，所以传统推荐模型其实很难达到比较好的scaling能力。

![nce-distribution-drift](<.gitbook/assets/nce-distribution-drift (1).png>)
...
![hstu-dlrm-vs-gr](<.gitbook/assets/hstu-dlrm-vs-gr (1).png>)
...
* 因为要自回归地算，可以理解为batch\_size也是$$n_i$$，加一个下三角的mask，所以在$$\sum$$里还要乘一个$$n_i$$
...
![hstu-arch](<.gitbook/assets/hstu-arch (1).png>)
...
使用了一种高效的attention kernel的GPU算子，类似FlashAttention，能够将**融合连续的矩阵操作**（fuse back-to-back GEMMs），但能够 进行fully raggified（可能是不规则，即序列长度可变的？？）的attention计算，本质是将attention计算转换为**不同大小的分组GEMMs**。因此，HSTU变成了memory-bound，并且能够以$$\Theta\left(\sum_i n_i^2 d_{q k}^2 R^{-1}\right)$$进行scale，其中，$$n_i$$是样本$$i$$的序列长度，$$d_{qk}$$是attention的维度，$$R$$是寄存器的大小。

受[Deep networks with stochastic depth](1.7.llm_recommend.md)启发，提出了SL(stochastic length)来增加用户历史序列的稀疏性，推荐系统中的用户行为往往有周期性，并且以不同形式呈现，因此引入稀疏性可以在效果不怎么损失的情况下显著减少encoder的代价，可以scale为$$\Theta\left(\sum_i n_i^2\right)$$
...
![gated-attention-unit](<.gitbook/assets/gated-attention-unit (1).png>)

假设$$d$$是embed size，$$h$$是head数，$$d_{q k}$$是attention的dim，$$d_{ff}$$是ffn的hidden size，

* 将一些计算融合成一个op，包括$$\phi_1\left(f_1(\cdot)\right)$$、layer\_norm、optional dropout和输出MLP，将每一层的激活的内存占用减小到了$$2 d+2 d+4 h d_{q k}+4 h d_v+2 h d_v=14 d$$（以bf16计算，一个参数2字节）---没懂

自己的理解：layer\_norm要存均值+方差，所以要$$2d\times 2bytes$$，所以上面式子就是，U($$2d$$)，V($$2d$$)，Q和K($$4hd_{qk}$$)，QKV的norm(因为有均值和方差，所以是$$4hd_v$$)，QKV($$2hd_v$$)

对比transformer，在attention后用了ffn和dropout，假设中间状态是$$3hd_v$$，那么ffn包括layer\_norm、linear、激活、linear、dropout，中间状态占用的就是$$2 d+4 d_{f f}+2 d+1 d=4 d+4 d_{f f}$$，一般来说，$$h d_v \geq d$$，$$d_{f f}=4 d$$，
...
![](<.gitbook/assets/m-falcon-mask (1).png>)
...
![hstu-scaling-law](<.gitbook/assets/hstu-scaling-law (1).png>)
...
![](<.gitbook/assets/unify-sid-id (1).png>)

![](<.gitbook/assets/unify-sid-id2 (1).png>)
...
![](<.gitbook/assets/prefix-ngram (1).png>)
...
![](<.gitbook/assets/vista-1 (1).png>)

![](<.gitbook/assets/vista-2 (1).png>)
...
![](<.gitbook/assets/kunlun-pffn (1).png>)
...
![](<.gitbook/assets/semi-local-attn (1).png>)

* bf16/fp8的混精度算子融合，同时适配AMD卡（可能能支持异构gpu训练？）

![](<.gitbook/assets/fuse-mixed-ultra-hstu (1).png>)
...
![](<.gitbook/assets/mtgr-tradition (1).png>)
...
![](<.gitbook/assets/mtfm (1).png>)

![](<.gitbook/assets/mtfm-mask (1).png>)
...
![](<.gitbook/assets/mbgr-label (1).png>)
...
![](<.gitbook/assets/HeteroRec (1).png>)
...
其中，重构loss如下，$$e$$是encoder的输入，$$z$$是decoder的输入，$$z_q$$是和$$z$$的cos最大的code（即下面的hard\_ind）对应的向量(即下面的soft\_ind)
...
![](<.gitbook/assets/next-interest-flow (1).png>)
...
![](<.gitbook/assets/grc-idea (1).png>)

![](<.gitbook/assets/grc (1).png>)
...
![](<.gitbook/assets/onerec-arch (1).png>)

![](<.gitbook/assets/onerec (1).png>)

![](<.gitbook/assets/onerec-kmeans (1).png>)

![](<.gitbook/assets/onerec-deploy (1).png>)
...
![](<.gitbook/assets/rankmixer (1).png>)
...
![](<.gitbook/assets/pinrec-demo (1).png>)
...
![](<.gitbook/assets/pinfm-serving (1).png>)
...
![](<.gitbook/assets/unipinrec-serving (1).png>)
...
实验： 基线：Pinrec，用ANN检索出K个候选，直接进排序(TransAct V2的精排) UniPinRec：先ANN（L0），再排序(L1)，得到K个候选，再进上面的排序
...
![](<.gitbook/assets/genrank-mask (1).png>)
...
![](<.gitbook/assets/grab1 (1).png>)

输入多个用户的序列，过统一的trans，然后目标好像不是自回归，应该是ranking

![](<.gitbook/assets/grab2 (1).png>)

序列压缩，把不同用户的序列合并成一条序列，然后加上对应的mask，保证用户间不可见，用户内causal

![](<.gitbook/assets/grab-partial-full-mask (1).png>)
...
![](<.gitbook/assets/gpr-training (1).png>)
...
![](<.gitbook/assets/s-grec (1).png>)

在GR里加了一个LLM as Judge产出语义reward，加上业务指标的reward一起RL

![](<.gitbook/assets/s-grec-frame (1).png>)
...
![](<.gitbook/assets/s-grec-rl (1).png>)
...
![](<.gitbook/assets/s-grec-advantage (1).png>)
...
![](<.gitbook/assets/oneranker-arc (1).png>)

![](<.gitbook/assets/oneranker (1).png>)
...
![](<.gitbook/assets/onepiece-frame (1).png>)

![](<.gitbook/assets/onepiece-context-engineering (1).png>)

![](<.gitbook/assets/onepiece-reasoning-mtl (1).png>)
...
![](<.gitbook/assets/climber-pilot (1).png>)
...
![](<.gitbook/assets/climber-pilot-serving (1).png>)
...
![](<.gitbook/assets/argus-pretrain (1).png>)
...
![](<.gitbook/assets/argus-finetune (1).png>)
...
![](<.gitbook/assets/capacity-constrained-rq (1).png>)
...
* STE（straight-through estimator）：直接更新整个码本，原始RQ-VAE是只更新argmax出来的那个code，这种稀疏更新方式很依赖码本的初始化。最原始的VQ-VAE论文([Neural Discrete Representation Learning](https://proceedings.neurips.cc/paper_files/paper/2017/file/7a98af17e63a0ac09ce2e96d03992fbc-Paper.pdf))里也用到了，另外[https://www.daiwk.net/1.2.llm\_intro#h-net](https://www.daiwk.net/1.2.llm_intro#h-net)这里也有。STE如下，其中$$\operatorname{sim}\left(\mathbf{h}_i^l, \mathbf{C}_l\right) \in \mathcal{R}^K$$表示cos相似度
...
![](<.gitbook/assets/snapchat-sid-gr (1).png>)
...
![](<.gitbook/assets/novelty-llm (1).png>)
...
拿惊喜度高的样本(一个context item\_id，next item\_id)，即$$(v_i,n_j)$$来sft一个MLLM，通过如下方式保证不同cluster的均衡
...
![](<.gitbook/assets/serendipitous-prompts (1).png>)
...
![](<.gitbook/assets/plum-cpt-data (1).png>)
...
![](<.gitbook/assets/static-algo1 (1).png>)

![](<.gitbook/assets/static-algo2 (1).png>)

### Google的LLM用户画像

[LLM-Based User Personas for Recommendations at Scale](https://arxiv.org/pdf/2606.12198)

![](<.gitbook/assets/llm-user-persona (1).png>)

![](<.gitbook/assets/llm-user-persona-arch (1).png>)
...
![](<.gitbook/assets/tokenminds1 (1).png>)

产出2个东西给下游用，一个是encoder输出的emb，一个是decoder输出的sid

![](<.gitbook/assets/tokenminds-enc-dec (1).png>)
...
![](<.gitbook/assets/tokenminds-multicontext-decoding (1).png>)

为了区分场景，在每个item前面加condition token，例如`<LFV>`(长视频)和`<SFV>`（短视频）

![](<.gitbook/assets/tokenminds-decoder-input (1).png>)

![](<.gitbook/assets/tokenminds-serving (1).png>)
...
![](<.gitbook/assets/llm-ad2ad-gen (1).png>)

![](<.gitbook/assets/llm-context-cate-graph (1).png>)
...
![](<.gitbook/assets/coea-prompts (1).png>)
...
![](<.gitbook/assets/userlm-r1 (1).jpg>)
...
*   Step 1：通用用户画像构建
    ...
    * 静态画像：背景信息、人格特质、表达风格、生活场景。基于AlignX（[From 1,000,000 Users to Every User: Scaling Up Personalized Preference for User-level Alignment](https://arxiv.org/pdf/2503.15463)）的数据集，把其中如下图的90维偏好向量和人口属性特征丢给LLM，通过不同prompt产出上述4种静态画像
    ...
    ![](<.gitbook/assets/profile-schema (1).png>)

    ![](<.gitbook/assets/alignx (1).jpg>)
...
![](<.gitbook/assets/lum-group (1).png>)
...
![](<.gitbook/assets/urm-item-emb (1).png>)
...
![](<.gitbook/assets/urm-prompt (1).png>)
...
![](<.gitbook/assets/recgpt-user-mining (1).png>)
...
![](<.gitbook/assets/recgpt-human-llm (1).png>)

![](<.gitbook/assets/recgpt-item-tag-pred (1).png>)
...
![](<.gitbook/assets/recgpt-item-retrieval (1).png>)
...
![](<.gitbook/assets/recgpt-explanation (1).png>)
...
![](<.gitbook/assets/choirrec-frame (1).png>)
...
![](<.gitbook/assets/choirrec-stage2 (1).png>)
...
![](<.gitbook/assets/choirrec-stage3 (1).png>)
...
![](<.gitbook/assets/sigma-scaling-deploy (1).png>)
...
![rec2llm-llm2rec](<.gitbook/assets/rec2llm-llm2rec (1).png>)

快手商业化把llm用到推荐的思路变化：

![](<.gitbook/assets/learn-roadmap (1).png>)

本文提出了LEARN(Llm-driven knowlEdge Adaptive RecommeNdation)，实现了LLM-to-Rec，让LLM作为content extractor，推荐任务是训练目标。

![learn-cex-pal](<.gitbook/assets/learn-cex-pal (1).png>)
...
![learn](<.gitbook/assets/learn (1).png>)
...
![](<.gitbook/assets/pinnerformer-loss (1).png>)
...
![](<.gitbook/assets/staytime-analysis (1).jpeg>)
...
![](<.gitbook/assets/r4ec-dataset (1).png>)
...
![](<.gitbook/assets/das-kuaishou (1).png>)
...
![](<.gitbook/assets/align3gr (1).png>)

![](<.gitbook/assets/align3gr-arch (1).png>)

![](<.gitbook/assets/behavior-modeling-level-align (1).png>)
...
![](<.gitbook/assets/glide-prompt (1).png>)
...
![](<.gitbook/assets/kar-prompts (1).png>)

看实验应该是用在召回，每个user和vec去找最像的k个item，但没法ann，只能暴力算

代码：[https://github.com/YunjiaXi/Open-World-Knowledge-Augmented-Recommendation/blob/main/knowledge\_encoding/utils.py](https://github.com/YunjiaXi/Open-World-Knowledge-Augmented-Recommendation/blob/main/knowledge_encoding/utils.py)，其实就是把模型输出的hidden states处理一下：
...
![notellm](<.gitbook/assets/notellm (1).png>)
...
![](<.gitbook/assets/hymirec1 (1).png>)

全序列量化后丢给一个轻量的推荐模型，产出兴趣向量，再把截断的短期行为和兴趣向量一起给LLM产出多兴趣的用户向量

![](<.gitbook/assets/hymirec2 (1).png>)
...
![ILM-tasks](<.gitbook/assets/ILM-tasks (1).png>)
...
![](<.gitbook/assets/2-stage-blip-2 (1).png>)

引入了Q-former

![ILM](<.gitbook/assets/ILM (1).png>)
...
![](<.gitbook/assets/blue-arch (1).png>)
...
![](<.gitbook/assets/embsum-summary (1).png>)
...
![](<.gitbook/assets/recoworld (1).png>)
...
![](<.gitbook/assets/recoworld-modeling (1).png>)

![](<.gitbook/assets/recoworld-mindset (1).png>)
...
![](<.gitbook/assets/recoworld-agentic-recsys (1).png>)
...
![slim](<.gitbook/assets/slim (1).png>)
...
![](<.gitbook/assets/text-features (1).png>)

整体的框架如下：

![](<.gitbook/assets/llm-cf-online-offline (1).png>)
...
![](<.gitbook/assets/llm-cf-total (1).png>)
...
![dllm2rec](<.gitbook/assets/dllm2rec (1).png>)
...
![](<.gitbook/assets/neo (1).png>)

![](<.gitbook/assets/neo-compare (1).png>)

![](<.gitbook/assets/neo-stage (1).png>)
...
ICLR2023，[https://github.com/kathrinse/be\_great](https://github.com/kathrinse/be_great)

通过自回归的LLM合成（synthesize）逼真的（realistic）表格化（tabular）数据

![](<.gitbook/assets/great-finetune (1).png>)
...
![](<.gitbook/assets/great-sample (1).png>)
...
![](<.gitbook/assets/llama-e-tasks (1).png>)
...
![](<.gitbook/assets/sagcn-graph (1).png>)
...
![](<.gitbook/assets/once-genre (1).png>)
...
![](<.gitbook/assets/agent4rec (1).png>)
...
![](<.gitbook/assets/recprompt (1).png>)
...
![](<.gitbook/assets/trans-rec-instruct-tuning (1).png>)
...
![](<.gitbook/assets/transrec-generation-grounding (1).png>)
...
![](<.gitbook/assets/llama4rec (1).png>)
...
![](<.gitbook/assets/llama4rec-prompts (1).png>)
...
![](<.gitbook/assets/guided-emb (1).png>)

一些相关prompt：

* aspect示例：

![](<.gitbook/assets/guided-emb-aspect (1).png>)

* item prompt：

![](<.gitbook/assets/guided-emb-item-prompt (1).png>)

* user prompt：

![](<.gitbook/assets/guided-emb-user-prompt (1).png>)

* user review示例：

![](<.gitbook/assets/guided-emb-user-review (1).png>)

* item示例：

![](<.gitbook/assets/guided-emb-item-example (1).png>)

### GMC

[Generative Multi-Target Cross-Domain Recommendation](https://arxiv.org/abs/2507.12871)

![](<.gitbook/assets/gmc (1).png>)
...
![](<.gitbook/assets/gream-onerecthink (1).jpg>)
...
![](<.gitbook/assets/mavrs-eval (1).png>)
...
![](<.gitbook/assets/mavrs-future (1).png>)
...
![](<.gitbook/assets/zeroshot-crossdomain-kd (1).png>)
...
![](<.gitbook/assets/auxiliary-head (1).png>)
...
![](<.gitbook/assets/sa-my-pic (1).png>)
...
![](<.gitbook/assets/llatte-query (1).png>)
...
![](<.gitbook/assets/monn-hill (1).png>)

![](<.gitbook/assets/monn (1).png>)
...
![](<.gitbook/assets/onelayer-attn-learning (1).png>)
...
![](<.gitbook/assets/3layer-monn (1).png>)
...
![](<.gitbook/assets/pre-materialization (1).png>)
...
![](<.gitbook/assets/versioned-late-materialization (1).png>)
...
![](<.gitbook/assets/rec-distill (1).png>)
...
* 迁移率的提升：
  ...
  * 蒸馏loss设计：mse比较不稳定，交叉熵更好；loss\_gt + alpha \* loss\_distill，不强制权重和为1，以gt为主，调distill的scale
  * 采样纠偏：teacher和student的采样逻辑不一致，解法是teacher落原始logit，蒸馏时，按student采样纠偏后的新logit当成student要学的目标

![](<.gitbook/assets/rec-distill-arch (1).png>)
...
![](<.gitbook/assets/decision-transformer (1).png>)
...
| 论文                                            | 公司     | 关键词                | 做法                                                                                                       |
| --------------------------------------------- | ------ | ------------------ | -------------------------------------------------------------------------------------------------------- |
| [TIGER](https://arxiv.org/pdf/2305.05065.pdf) | google | semantic\_id粒度的生成式 | <p>- 基于RQ-VAE聚类<br>- 输入semantic_id list，预测下一个semantic_id<br></p>                                         |
| [HSTU](https://arxiv.org/pdf/2402.17152.pdf)  | meta   | item\_id粒度的生成式     | <p>- 性别年龄等profile特征、action type也可以作为特殊的item_id<br>- 输入item_id list，预测下一个item_id<br>- 一种范式可以同时支持召回和排序</p> |
...
![gecko](<.gitbook/assets/gecko (1).png>)
...
![fret](<.gitbook/assets/fret (1).png>)
...
![chatretriever](<.gitbook/assets/chatretriever (1).png>)
...
![](<.gitbook/assets/unime-text-distill (1).png>)
...
![](<.gitbook/assets/unime-img (1).png>)
...
![](<.gitbook/assets/qwen3-emb-arch (1).png>)
...
![](<.gitbook/assets/qwen3-emb-train (1).png>)
...
![](<.gitbook/assets/t5gemma-embgemma (1).png>)
...
![](<.gitbook/assets/sira-compare (1).png>)

![](<.gitbook/assets/sira (1).png>)
...
![](<.gitbook/assets/mixlm1 (1).png>)

![](<.gitbook/assets/mixlm2 (1).png>)
...
![](<.gitbook/assets/slm-ranking (1).png>)
...
![](<.gitbook/assets/slm-ranking2 (1).png>)
...
![](<.gitbook/assets/slm-ranking-ablation (1).png>)

还做了loss mask，即一些很sparse的正例很容易被淹没，因此对于每个正例，只保留它所在那些query的样本来做负例，如图

![](<.gitbook/assets/slm-ranking-loss-mask (1).png>)
...
![](<.gitbook/assets/lightretriever (1).png>)
...
![](<.gitbook/assets/lightretriever-dense (1).png>)
...
![](<.gitbook/assets/lightretriever-sparse (1).png>)
...
![](<.gitbook/assets/self-evolving-recsys (1).png>)
...
![](<.gitbook/assets/self-evolving-recsys-prompt (1).png>)

![](<.gitbook/assets/self-evolving-recsys-modelsize (1).png>)
...
![](<.gitbook/assets/agenticrectune (1).png>)

Reasoning Loop：

actor和critic的prompt：

![](<.gitbook/assets/actor-critic-prompt (1).png>)
...
![](<.gitbook/assets/gears (1).png>)

![](<.gitbook/assets/gears-algo (1).png>)

![](<.gitbook/assets/gears-2 (1).png>)

### 虾皮的Sortify

[Let the Agent Steer: Closed-Loop Ranking Optimization via Influence Exchange](https://arxiv.org/pdf/2603.27765v1)

![](<.gitbook/assets/sortify1 (1).png>)

![](<.gitbook/assets/sortify2 (1).png>)

![](<.gitbook/assets/sortify-arch (1).png>)
...
![](<.gitbook/assets/sortify-llm (1).png>)
...
![](<.gitbook/assets/mtmh-serving-content (1).png>)
...
![](<.gitbook/assets/mfli1 (1).png>)

LI（learnable index）就是在训练的时候建好索引，参考当年的tdm

![](<.gitbook/assets/mfli (1).png>)
...
![](<.gitbook/assets/mfli-rebalancing (1).png>)

![](<.gitbook/assets/mfli-index-structure (1).png>)
...
![](<.gitbook/assets/meta-adaptive-ranking-model (1).png>)

![](<.gitbook/assets/meta-adaptive-ranking-model2 (1).png>)
...
![](<.gitbook/assets/dgenctr (1).png>)

![](<.gitbook/assets/dgenctr-model (1).png>)
...
![](<.gitbook/assets/store-1epoch (1).png>)
...
![](<.gitbook/assets/est-multi-epoch (1).png>)
...
![](<.gitbook/assets/pinterest-2-stage (1).png>)
...
![](<.gitbook/assets/multi-faceted-1 (1).png>)

红色部分的item id的emb用的是上面那篇paper的2阶段方法，即下图的结构

![](<.gitbook/assets/multi-faceted-2 (1).png>)
...
![](<.gitbook/assets/multi-faceted-3 (1).png>)
