...
+ 右边：2~T+1个item，每一种特征把2~T+1拼到一起过各自的tower，每个tower得到的感觉还是2~T+1个输出，这个输出和左边的token粒度的输出List算个LMP，然后过一个item cross tower，我猜输出的是2~T+1个item表示，和左边的item粒度输出List再算一个LMP
+ LMP：左边是1~T，右边是2~T+1，t1过个mlp预测t3，t1过另一个MLP预测t2，即multi-step预估
