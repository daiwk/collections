...
+ 融合：$$A^TB$$可以算出一个$$m\timesn$$的相似矩阵，然后flatten，得到一个$$mn$$维的向量，同理算一下，得到的$$s^i_j$$就是两个向量的内积，即相似度的得分了
...
|[Can Small Language Models be Good Reasoners for Sequential Recommendation?](https://arxiv.org/pdf/2403.04260) SLIM | 蚂蚁 | 蒸馏推荐理由| 输入用户行为历史，大LLM(gpt)产出的推荐理由；小llm(llama2)去蒸馏这个理由拿小llm去给出全量user的推荐理由，通过BERT得到emb | 广告cvr+收入 |
