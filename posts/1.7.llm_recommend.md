...
+ 不是直接用Gemma 3这种decoder-only的模型，而是先把它改造成一个encoder-decoder架构(用如上图的T5-gemma的方法，[Encoder-decoder gemma: Improving the quality-efficiency trade-off via adaptation](https://arxiv.org/abs/2504.06225))。然后再从这个架构里把encoder抽出来作为EmbeddingGemma的底座。并且内容中的消融研究证明了这种方式初始化的模型比直接用decoder-only的或者随机初始化的效果要好得多，因为encoder主要做内容理解，表征能力更强。
+ 蒸馏：用更大更强的Gemini Embedding模型当老师，通过embedding matching的几何蒸馏方法，直接对齐两个模型的嵌入空间。
+ 用了spread-out的正则化损失函数，目的是让生成的向量在嵌入空间里散得更开，充分利用整个空间的表达能力。这样做的好处是模型对量化更robust，而且用最近邻检索时效率也更高。
