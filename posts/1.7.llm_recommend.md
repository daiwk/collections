...
问题：如何支付广告费用？
输出：
广告按点击收费。费用因广告位置和点击次数而异。不超出预算，达到预算上限时广告停止。
```
...
图中的C是customer，S是seller，P是platform

+ instruction formulating: 
...
设置一系列的task(100多个task)来finetune LLM
...
|[TransRec](https://arxiv.org/pdf/2310.06491) |  | - 将一个item表示成3部分：id+title+attr，设计三种对应的instruct-tuning任务；<br> - 引入一个特殊的数据结构（FM-index），并进行constrained beam search，让模型能生成候选集中的id/title/attr，<br> 再遍历全库候选，看不同facet的相似度（会考虑高热打压），加权融合出一个排序| lora|
