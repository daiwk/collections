...
+ 异构信息分解（HTFL）：把物品的各种信息（比如文字描述、图片、价格）拆分成多个小块（称为“token”），就像把一句话拆成单词一样。然后把这些小块按用户行为的时间顺序排列，形成一个更丰富、更灵活的序列。这样可以让模型更细致地理解物品之间的关系，还能通过增加模型层数来提升效果。
+ 层级因果Transformer（HCT）：用一个特别设计的Transformer模型，先分析这些小块之间的细微联系，再综合成用户的整体兴趣。这种分层处理方式能捕捉到更深层次的模式，比如“用户为什么喜欢某个东西”。
...
+ ExFM：
    ...
    + DAS, data augmentation service：FM对每个VM数据进行预估，再加上对应的VM数据
    + Auxiliary Head和Student Adapter：t-1的VM模型和t的VM模型
...
新的Transfer Scaling Law：当不断迭代和提升基础大模型的模型规模时，其高效的知识转化率使得线上的广告排序模型的性能呈现出连续数年的持续提升，且增速在不断扩大。
...
探究了大模型架构在序列推荐中参数量与效果的关系，并提出了一种知识蒸馏的方法，使得小语言模型仅用13%的模型参数，取得了比现有大模型推荐方法略好的效果，并取得了在训练/推理阶段6.6x/8.0x的加速。
