...
![](../assets/semi-local-attn.png)

+ bf16/fp8的混精度算子融合，同时适配AMD卡（可能能支持异构gpu训练？）

![](../assets/fuse-mixed-ultra-hstu.png)
 
## Meta的SlimPer

[SlimPer: Make Personalization Model Slim and Smart](https://arxiv.org/pdf/2607.12281v1)
...
+ K/V：全部user-side tokens，数量N在千到万的量级，
+ Q：item侧或者cross的特征，搞出K个token，k大概是16到64之间

过程：

+ 先Q和K/V做一次cross attn，得到一个qxd的向量；序列特征和sparse特征(都是作为K/V)分别搞一下，得到2个这种向量
+ 从原始Q搞出t个template，即txd，和上面cross attn的结果算点积，得到txq的lambda，即2个lambda
+ 然后做一个refine的操作，这两个lambda，还有其他的一些dense特征，concat并过mlp映射到K维，再残差一下，得到下一层输入

## Meta的Mosaic

[Mosaic: A Fleet of User Embedding Specialists for Recommendation at Meta](https://arxiv.org/pdf/2607.24015)

![](../assets/mosaic.png)

不是只训练一个通用用户embedding模型，而是维护一组不同类型的用户embedding专家模型，每个专家负责捕捉用户行为的一个侧面，然后把这些embedding作为标准特征喂给下游retrieval、ranking、integrity(平台治理/生态安全之类的)等模型。

四类专家：

+ Memorization-driven：用超大embed hash table记住细粒度用户偏好，各种纯id类特征。
+ Dense-heavy：处理大量聚合统计特征，比如1d/7d/28d的点赞、评论、分享、跨场景计数，用MHTA([Multi-Head Talking Attention](https://arxiv.org/pdf/2003.02436))、SEDot([Squeezeand-Excitation Networks](https://arxiv.org/pdf/1709.01507))、Wukong这类结构建模非线性交叉。
+ Sequential：建模用户长行为序列，基于HSTU/attention，加了上下文token、内容/行为metadata拆分token、层级attention、MoE等。
+ CoTrain：把下游ranker的梯度也接进用户塔，让embedding更贴近某个具体业务面的目标。

前3个专家是上游的多任务训练，第4个专家是下游任务去训练

![](../assets/mosaic-arch.png)

有没有target信息？

+ training-only architecture：可以吃non-user features，包括item/context相关特征，并用engagement label训练。
+ user tower：吃用户侧特征，输出一个或多个 user embedding。
+ 训练时non-user分支的loss/gradient 会反传到user tower，让user embedding学到对下游任务有用的表示。
+ inference时只部署user tower，只给用户侧输入

serving：

+ 低成本/轻模型/可在线现算：CPU online
+ 高freshness/重模型/必须现算：GPU online，用如下方式降成本：
    + AOTI：PyTorch的Ahead-of-Time编译路径，线上推理不要每次走Python eager runtime，而是提前把模型图编译好，并尽量fuse operator，减少Python调度、kernel launch、算子间开销
    + predictor tuning：
        + batch coalescing：把短时间内多个请求合成batch，提高GPU利用率，但要控制等待时间。
        + thread-pool sizing：线程池大小要合适，太小吃不满，太大会争抢CPU、调度变慢。
        + intra/inter-op parallelism：控制单个算子内部并行和多个算子之间并行，避免线上推理过度并发导致抖动。
    + model split：把模型拆成不同子路径，让sparse embedding部分和dense/transformer部分走更适合自己的执行方式，而不是整个模型硬塞进一个统一执行图里。
    + memcache：用户embedding不会每秒剧烈变化，很多用户还会在短时间内多次请求。于是线上先查cache，TTL是2小时
+ 大模型/freshness可放宽/全量用户可周期更新：GPU offline batch inference，写embedding index，线上读表
...
![](../assets/next-interest-flow.png)

+ 生成式预训练：拿判别式模型（transformer）来初始化decoder的权重，输入历史序列，预估未来的T个item，在正常的生成式loss之外还加了：
    + Velocity：限制模型预估的不要太跳跃，即相邻item尽量像一点：$$\mathcal{L}_{\mathrm{vel}}=\sum_{t=t_0+1}^{t_0+T-1}\left\|\left(\hat{\mathbf{f}}_t-\hat{\mathbf{f}}_{t-1}\right)-\left(\hat{\mathbf{f}}_{t-1}-\hat{\mathbf{f}}_{t-2}\right)\right\|_2^2$$
    + diversity：h个attention head两两的相似度尽量小：$$\mathcal{L}_{\mathrm{div}}=\sum_{t=t_0}^{t_0+T-1}\left(\frac{1}{\binom{H}{2}} \sum_{i=1}^{H-1} \sum_{j=i+1}^H\left(\operatorname{sim}\left(\mathbf{h}_i, \mathbf{h}_j\right)\right)^2\right)$$
...
+ 判别式增强（TSP，Temporal Sequential Pairwise）：对calibration net加的辅助loss
    ...
    + 但实际loss就是个bpr，即给定一个pair对，正样本的预估分比负样本高得越多越好，而因为每个item是根据历史序列得到的预测，所以变相地鼓励模型通过历史序列来隐式地归因：$$\mathcal{L}_{\text {tsp }}=-\frac{1}{\left|\mathcal{D}_{\text {paired }}\right|} \sum_{\mathcal{D}_{\text {paired }}} \log \sigma\left(\mathbb{I}\left(y_{t_1}\right)\left(c_{t_1}-c_{t_0}\right)\right)$$
...
![](../assets/onerec-arch.png)

![](../assets/onerec.png)

![](../assets/onerec-kmeans.png)

![](../assets/onerec-deploy.png)
...
+ mixup转置，new_tokens假设是16，那每个新token就是$$24\times 512/16=768$$维
...
第二层也是先mixup转置，例如你设置8个new_tokens，那就对应8个独立的FFN
...
[UniPinRec: Unifying Generative Retrieval and Ranking at Pinterest Scale](https://arxiv.org/pdf/2606.00422)

生成式(GR/TIGER)的问题：

>Meta的两篇论文[How Well Does Generative Recommendation Generalize?](https://arxiv.org/pdf/2603.19809)和[Unifying Generative and Dense Retrieval for Sequential Recommendation](https://arxiv.org/pdf/2411.18814)(LIGER)

+ lossy compression：item信息被压成几个离散token，细粒度信息会丢
+ generation objective inefficient：next-token生成不是最适合topK retrieval的目标
+ cold-start failures：理论上semantic ID有冷启动优势，但实际beam search可能召不出来，因为新item没有交互，模型没有见过它作为target，自然容易给它低概率。
+ token relations memorized in-parameter：所谓“泛化”很多时候是prefix token级记忆，GR看起来的item-level generalization，很多可以解释成 token-level memorization。即模型不是凭空理解了item之间的复杂关系，而是因为不同item共享semantic ID prefix，模型记住了prefix transition

![](../assets/unipinrec.png)

本文将召回和粗排(感觉应该算是召回内的rerank)合到一个模型里，2个head负责不同工作，还是item id粒度的建模，并没有用sid

![](../assets/unipinrec-arch.png)

+ 输入表示：将action emb和item emb concat在一起
+ mask机制：
    + 历史序列里：以一定概率mask掉action，用```[mask]```替换
    + 候选item及未来item的action：全部用```[mask]```
+ attention：类似m-falcon的一个causal mask，即候选可以看见所有历史的，但候选之间不可见（注：图中的attn mask里深色是可见的）
+ 训练目标：
    + next-item predict（召回loss）：对每个没mask的历史位置，算samlped softmax
    + action predict（排序loss）：h是一个mlp，映射到C个actoin，$$\mathcal{M}_{\text {past }}=\left\{i: m_i=0\right\}$$是历史里mask掉的那些item

$$
\begin{gathered}
\mathcal{L}_{\text {action }}=\sum_{c=1}^C w_c\left[\frac{1}{\left|\mathcal{M}_{\text {past }}\right|} \sum_{i \in \mathcal{M}_{\text {past }}} \ell_{\mathrm{BCE}}\left(h_{\psi_c}\left(z_i\right), a_i^{(c)}\right)\right. \\
\left.+\frac{1}{k} \sum_{j=1}^k \ell_{\mathrm{BCE}}\left(h_{\psi_c}\left(z_j^{\prime}\right), a_j^{\prime(c)}\right)\right],
\end{gathered}
$$

+ 优势：
    + 召回和排序的序列是一样的
    + 召回和排序的模型权重是可以share的，排序可以直接用召回的ckpt初始化
    + random mask加强泛化能力

![](../assets/unipinrec-serving.png)

他们的召回和排序服务是部署在同一机器/卡上的不同进程，感觉需要将同一个请求的召回/排序请求打到同一机器/卡上。

主要是排序复用召回的shared KV pool。一般跨进程的kvcache传输会有d2h再h2d的开销，所以做了一个shared GPU memory pool：

+ 每个召回服务预分配一个$$[L,S,H,n,D]$$大小的gpu pool，对应(layer数，slot数(一台机器/一张卡缓存的请求数)，kv heads, 历史序列长度, head dim)，然后通过CUDA IPC handles暴露给排序服务。
+ 排序服务打开这些handle，把同一块GPU allocation映射到自己的地址空间。slot会round-robin复用。

实验：
基线：Pinrec，用ANN检索出K个候选，直接进排序(TransAct V2的精排)
UniPinRec：先ANN（L0），再排序(L1)，得到K个候选，再进上面的排序

场景：Board More Ideas(BMI，围绕某个收藏夹/画板继续推荐item)和push场景，有收益
...
![](../assets/grab-partial-full-mask.png)

单用户内部，每一个item又拆成了2个token：

+ partial token：图中的h，只保留随时间变化、对刻画历史有用的信息（例如context、相对位置/时间），把静态或高度重复的字段丢掉（例如user_id、长期稳定兴趣），只能看到t-1时刻往前的partial和自己
...
![](../assets/oneranker-arc.png)

![](../assets/oneranker.png)
...
![](../assets/onepiece-context-engineering.png)
...
![](../assets/climber-pilot-serving.png)
...
![](../assets/capacity-constrained-rq.png)
...
$$
p(\mathbf{f} \mid y, u)=\prod_{k=1}^n p\left(f_k \mid f_{<k}, y, u\right)
$$
...
+ infer引入bid敏感性
    + ```<AD>```这个词的logit加上所有广告候选的max bid做扰动：$$\tilde{z}_{\langle\mathrm{AD}\rangle}=z_{\langle\mathrm{AD}\rangle}+\lambda \cdot \log \left(1+b_{\max }\right)$$
    + 后面的sid在decoding的时候，每一级sid都加上当前cluster的max bid做扰动：$$\tilde{z}_c=z_c+\lambda \cdot \log (1+\mathcal{B}(c))$$
...
+ 共现对比学习正则：一条样本是一个共现pair，在batch内算softmax，$$\mathcal{L}_{\text {con }}=-\sum_{i=1}^{2 N_b} \frac{\exp \left(\operatorname{sim}\left(p_i, p_i^{+}\right)\right)}{\sum_{j=1}^{2 N_b} \exp \left(\operatorname{sim}\left(p_i, p_j\right)\right)}$$，
+ 最终loss就是重建Loss+rq_loss+共现正则

![](../assets/plum-cpt-data.png)
...
sft如图，输入有文本也有sid，预测下一个sid。设计了带reward（人工为每个点击拍的）的loss：$$\mathcal{L}_{\mathrm{SFT}}=-\sum_{t=1}^L r\left(\text { user, } v_{c l i c k}\right) \cdot \log P\left(\text { sid }_t \mid \text { Context }_{\text {user }}, \text { History }_{\text {user }}, \text { sid }_{<t}\right)$$
...
[https://github.com/youtube/static-constraint-decoding](https://github.com/youtube/static-constraint-decoding)，有jax和pytorch版本
...
![](../assets/static-algo1.png)

![](../assets/static-algo2.png)

## Google的LLM用户画像

[LLM-Based User Personas for Recommendations at Scale](https://arxiv.org/pdf/2606.12198)

![](../assets/llm-user-persona.png)

![](../assets/llm-user-persona-arch.png)

## Google的TokenFectory

[Token Factory: Efficiently Integrating Diverse Signals into Large Recommendation Models](https://arxiv.org/pdf/2606.19635)

![](../assets/tokenfactory-plum.png)

![](../assets/tokenfactory.png)

![](../assets/tokenfactory-user-history.png)
...
→ concat
→ MLP 或 Transformer
→ 固定数量的 d_model 维 soft tokens
→ 与标题等文本 token 一起输入 LRM
```

## Google的TokenMinds

[TokenMinds: Pretrained User Tokens and Embeddings for User Understanding in Large Recommender Systems](https://arxiv.org/pdf/2606.25147)

复用PLUM的CPT流程，训了个LLM

![](../assets/tokenminds1.png)

产出2个东西给下游用，一个是encoder输出的emb，一个是decoder输出的sid

![](../assets/tokenminds-enc-dec.png)

sid有8级，但拿来训练的只有前L=4级

+ encoder：用CPT的模型初始化，每个item有sid+1个softtoken（属性（时长，完播率，操作系统等）过mlp后的emb），还加了搜索序列，input长度最长1024
+ decoder输入：用CPT的模型初始化，未来24小时的随机观看序列（最多15个），仅sid，当做Q，和encoder输出做cross attn

![](../assets/tokenminds-multicontext-decoding.png)

为了区分场景，在每个item前面加condition token，例如```<LFV>```(长视频)和```<SFV>```（短视频）

![](../assets/tokenminds-decoder-input.png)

![](../assets/tokenminds-serving.png)
...
研究动机: 虽然LLM在推荐系统的召回和粗排阶段已展现潜力，但作为模型输出最终质量关口的“重排序（Re-ranking）”阶段却长期被忽视。现有LLM推荐方法多采用零样本或简单微调，未充分利用模型的推理（Reasoning）能力；且依赖非语义 ID，难以扩展至工业级数十亿规模。​

GR2(Generative Reasoning Reranker)：通过三阶段训练流水线赋能重排。​

+ 语义ID知识对齐(Mid-training)：采用RQ-VAE分层量化技术将文本特征转化为sid，并通过对比学习编码用户共参与（Co-engagement）信号。将SID与自然语言混合，对学生LLM（如 Qwen3-8B）进行mid-training，使其理解sid蕴含的物品语义。​
+ 推理链知识蒸馏(Reasoning SFT)：利用性能更强的教师LLM（如Qwen3-32B）针对重排任务生成高层级推理链（CoT），包含用户历史总结、类目偏好分析及候选物品匹配逻辑。通过拒绝采样筛选高质量推理路径，并对学生模型进行有监督微调。​
+ 策略优化与奖励校准(RL-DAPO)：基于解耦剪裁和动态采样策略优化（DAPO），利用可验证奖励（Verifiable Rewards）进行强化学习。通过惩罚模型“简单保留原始顺序”的Reward Hacking行为，强制其深入理解意图进行重排。


## Meta的LLM召回

[LLM Retrieval for Stable and Predictable Ad Recommendations](https://arxiv.org/pdf/2605.21969)

![](../assets/llm-ad2ad-gen.png)

![](../assets/llm-context-cate-graph.png)

## Meta的ConnectionMind

[ConnectionMind: Leveraging Social Networks and Large Language Models for Personalized Recommendation at Meta](https://arxiv.org/pdf/2608.10187)

![](../assets/connectionmind-graph.png)

异构图：节点包括用户、Page/创作者、Item/视频内容。
...
+ 用户和Page：关注创作者或主页。
+ 用户和Group：加入或互动的群组。
+ Group和Item：群组内分享/讨论的内容。
+ Page和Item：创作者发布的内容。
+ Item和Item：共看关系、语义相似关系。

视频会通过 OCR、ASR、视频理解模型提取文本/视觉/音频信息，再总结成紧凑文本；用户和创作者也会由历史互动内容生成语义profile。这样LLM在图上走路径时，不只是看结构，还能理解节点语义。

![](../assets/connectionmind.png)

推理过程：

+ 对目标用户采样一个紧凑的局部子图。
+ LLM看到当前路径、frontier节点的可选typed edges、节点文本描述。
+ LLM输出结构化动作，比如要扩展哪些路径、要surface哪些item。
+ 最终得到推荐item集合，以及每个item对应的证据路径。

训练：

+ SFT：用历史正反馈行为构造从用户到正样本item的最短路径，让LLM学会合法地走图、产出结构化路径。
+ RL：用GRPO，用规则奖励继续优化完整rollout，奖励包括格式合法性、最终推荐 F1，以及中间步骤是否朝可达正样本方向探索。

![](../assets/connectionmind-serving.png)

serving：

+ 对最活跃的top 5-10%用户，直接用ConnectionMind LLM生成显式推理路径。
+ 对大多数普通用户，用LLM离线发现有效meta-path，再蒸馏训练一个轻量Student GNN，线上毫秒级服务。
...
$$
P\left(v_i \mid \mathcal{T}\right)=P\left(c_1^k, \ldots, c_{\ell}^k \mid \mathcal{T}\right) \times P\left(i d_i \mid \mathcal{T}, c_1^k, \ldots, c_{\ell}^k\right)
$$

第一项是sid的beam score，第二项如下，其中的$$\sigma$$是前k个beam score的标准差，对于有约束的任务，要求相关性高，一般标准差较大，对于要求多样性的任务，则标准差小

$$
P\left(i d_i \mid \mathcal{T}, c_1^k, \ldots, c_{\ell}^k\right)=\frac{e^{\cos \left(\boldsymbol{h}_k, \tilde{\boldsymbol{v}}_i\right) \cdot \sigma\left(\left\{\phi_1, \ldots, \phi_K\right\}\right) / \tau}}{\sum_{v_j \in \mathcal{V}_k} e^{\cos \left(\boldsymbol{h}_k, \tilde{\boldsymbol{v}}_j\right) \cdot \sigma\left(\left\{\phi_1, \ldots, \phi_K\right\}\right) / \tau}}
$$

![](../assets/sigma-scaling-deploy.png)

实验的时候是加了一路近线召回

## 阿里的DREAM

[DREAM Technical Report](https://arxiv.org/pdf/2608.09408)

![](../assets/dream1.png)

![](../assets/dream-frame.png)
...
[https://github.com/wangshy31/OneRec-Think](https://github.com/wangshy31/OneRec-Think)
...
数据集：[https://huggingface.co/datasets/OpenOneRec/OpenOneRec-RecIF](https://huggingface.co/datasets/OpenOneRec/OpenOneRec-RecIF)
...
![](../assets/align3gr-arch.png)

![](../assets/behavior-modeling-level-align.png)
...
$$
e_j^{i+1}=e_j^i-\frac{e_j^i \cdot c_{b_j}^i}{\left\|c_{b_j}^i\right\|^2} \cdot c_{b_j^i}^i, j \in[1, N]
$$
...
[Bridging Textual Profiles and Latent User Embeddings for Personalization](https://arxiv.org/pdf/2605.06981)

![](../assets/blue.png)

![](../assets/blue-arch.png)
...
[GR2 Technical Report](https://arxiv.org/pdf/2606.31984)

## Meta的AMBER

[An Event is Worth One Token: Event Tokenization for Industrial-scale LLM Recommendation](https://arxiv.org/pdf/2608.25546)

一次曝光/交互发生时的user、item、context、cross和outcome等完整快照压成连续的Event Token，再让User LLM对Event Token序列做自回归ranking或retrieval。

+ 训练：Event Tokenizer与User LLM联训；
+ serving：在事件发生后异步计算一次并缓存

但没写ab收益

![](../assets/amber.png)

![](../assets/amber-2tasks.png)

其实和字节的ins-as-token非常像，主要区别是同时支持召回+排序task，以及user侧用了llm。文中说llm是一个预训练的1b llama初始化的，而且效果比随机初始化要好，但其实它的输入都是event-token，感觉不需要啥世界知识？下图也可以看出，一开始收益很大，但20k steps后就很小了其实。

![](../assets/amber-pretrain-random.png)
...
![](../assets/neo-compare.png)

![](../assets/neo-stage.png)
...
[https://github.com/AkaliKong/MiniOneRec](https://github.com/AkaliKong/MiniOneRec)

[https://huggingface.co/kkknight/MiniOneRec](https://huggingface.co/kkknight/MiniOneRec)
...
![](../assets/zeroshot-crossdomain-kd.png)
...
![](../assets/onelayer-attn-learning.png)
...
[Versioned Late Materialization for Ultra-Long Sequence Training in Recommendation Systems at Scale](https://arxiv.org/pdf/2604.24806)

之前的架构，全量序列特征dump到UIH Store，然后去训练和infer。当序列非常长的时候，肯定不太行。

![](../assets/pre-materialization.png)

新的架构，序列特征拆成2部分，immutable(不可变)的长期行为，mutable的短期行为，

在线落训练样本时只落短期行为，以及长期行为的version metadata；训练数据预处理时根据metadata自动查immutable store，再和短期行为join。

![](../assets/versioned-late-materialization.png)

目前已经成了foundation infra，包括HSTU和ULTRA-HSTU都在用

## Meta的IEFF

[Intelligent Elastic Feature Fading: Enabling Model Retrain-Free Feature Efficiency Rollouts at Scale](https://arxiv.org/pdf/2605.00324)

![](../assets/ieff.png)

特征相关的一个基建

+ Serving-time Feature Adapter：位于特征生成之后、模型推理之前。对于需要渐变的特征，会根据一个全局的渐变策略动态调整特征的覆盖率或分布。例如，要下线一个特征，可以每天将其覆盖率降低1%，而不是一次性降到0。
+ Centralized Control Plane：负责管理所有特征的渐变策略，包括渐变的起止时间、速率、目标值等，工程师写配置就行。
+ 经过adapter调整后的特征值，不仅会用于在线的模型推理，也会被记录下来用于后续的模型持续训练。
+ 安全与治理机制：
  + 预发布验证：在正式发布前，通过小流量A/B测试（QRT）验证渐变策略的安全性，确定合适的渐变速率。
  + 监控与自动回滚：持续监控核心指标（如模型 NE、业务指标），一旦超出安全阈值，可自动暂停或回滚渐变。
  + 可逆性：所有渐变策略都可以随时暂停或反向执行，实现快速回滚，且整个过程同样无需重训。

## Meta的Memento

[Memento: Personalized RAG-Style Long-Retention Data Scaling for Online Ads Recommendation](https://arxiv.org/pdf/2605.24051)

![](../assets/memento.png)

+ RAG阶段：用user emb+ad emb+context特征作为query Q，去用户历史的doc倒排/向量索引里找出一些D出来，然后按MMR排序（sim uu指q里的u部分，和d算相似度，sim ua指q里的ad部分，和d算相似度）

$$
\begin{aligned}
\mathrm{MMR}= & \underset{D_i \in R \backslash S}{\arg \max }\left[\alpha \cdot \operatorname{Sim}_{U U}\left(D_i, Q\right)\right. \\
& +\beta \cdot \operatorname{Sim}_{U A}\left(D_i, Q\right) \\
& \left.-(1-\alpha-\beta) \cdot \max _{D_j \in S} \operatorname{Sim}_{U U}\left(D_i, D_j\right)\right]
\end{aligned}
$$
...
[LoopFM: Learning frOm HistOrical RePresentations of Foundation Model for Recommendation](https://arxiv.org/pdf/2605.29280)

![](../assets/loopfm.png)

+ stage1：embedding extraction：对于每一个**样本**，FM有$$M$$层$$l_1, \ldots, l_M$$，取其中$$K$$层出来，concat到一起得到$$\mathbf{e}^{(i)}=\left[\mathbf{h}_{l_1}^{(i)} ; \ldots ; \mathbf{h}_{l_K}^{(i)}\right] \in \mathbb{R}^D$$，其中$$D=\sum_{k=1}^K d_{l_k}$$，大致在$$O\left(10^5\right)-O\left(10^6\right)$$
+ stage2：compression：原始的$$\mathbf{e}^{(i)} \in \mathbb{R}^D$$直接存储太大，用一个和FM联合训练的autoencoder来压缩成$$\mathbf{z}^{(i)} \in \mathbb{R}^d(d \ll D)$$（别的姜维方法也行，例如PCA、随机投影、可学习的codebook）：

$$
\mathbf{z}^{(i)}=f_{\mathrm{enc}}\left(\mathbf{e}^{(i)}\right), \quad \hat{\mathbf{e}}^{(i)}=f_{\mathrm{dec}}\left(\mathbf{z}^{(i)}\right), \quad \mathcal{L}_{\mathrm{AE}}=\left\|\mathbf{e}^{(i)}-\hat{\mathbf{e}}^{(i)}\right\|_2^2
$$

注意，autoencoder的梯度会stop-grad到FM的backbone。encoder最后一层用tanh映射到$$[-1,1]$$，然后用int4量化$$z_{\text {quant }}=\operatorname{round}(z \cdot 8) \cdot \operatorname{clamp}(-8,7) / 8$$，能比fp16节省4倍存储

还用了Matryoshka compression，$$\mathcal{L}_{\mathrm{MAE}}=\sum_{d^{\prime} \in \mathcal{D}}\left\|\mathbf{e}-f_{\operatorname{dec}}^{\left(d^{\prime}\right)}\left(\mathbf{z}_{1: d^{\prime}}\right)\right\|_2^2$$

+ stage3：structuring：key $$k$$可以是userid、itemid sementicid，本文用的是userid，截取往前一个时间窗口（例如30天）的最多$$L$$(例如200)个item：

$$
\mathbf{S}_k=\left[\mathbf{z}_{k, t_1}, \mathbf{z}_{k, t_2}, \ldots, \mathbf{z}_{k, t_L}\right], \quad t_L<\cdots<t_2<t_1<t_{c u r}
$$

VM-side integration：VM拿到$$\mathbf{S}_u$$后，过一个序列encoder(可以是pooling或者attn)，然后把pool结果和其他特征concat一起过后面的网络，loss包括task loss和kd loss：$$\mathcal{L}=\mathcal{L}_{\text {task }}\left(\hat{y}^V, y\right)+\lambda \cdot \mathcal{L}_{\mathrm{KD}}\left(\hat{y}^V, \hat{y}^F\right)$$，其中$$\hat{y}^V=g\left(\mathbf{x}_{\mathrm{VM}}, \mathbf{S}_u ; \Theta_V\right)$$


总结：

+ 每条样本存k层的hidden（融合了u+i的各种组合信息），压缩后得到z；
+ user level取前L个样本的z，拼起来给VM用

消融了z的dim和VM中序列encoder的$$d_{model}$$的关系：

1. 小的$$d_{model}$$限制迁移：固定在小$$d_{model}$$时，增加dim并没有收益，说明$$d_{model}$$需要足够大
2. dim限制信息容量：固定在小dim时，增加$$d_{model}$$只能有非常小的提升，因为本身携带的信息有限
3. 这两个维度需要同时scale：同时scale时，收益最大

## 字节的Rec-Distill

[Rec-Distill : An Industrial Distillation Pipeline for Large-Scale Recommendation Models](https://arxiv.org/pdf/2605.29755)

![](../assets/rec-distill.png)

精排模型不断scale up会遇到ROI问题，因此用蒸馏的思路，让更大的精排当teacher（batch+流式训但不serving），线上精排当student。

提升效果的核心思路：

+ teacher的scale up：mixer变大（最大可以到24b）+序列扩长（最长可以到2w）+增加训练数据
+ 迁移率的提升：
    + student拆成主塔+辅助塔：主塔正常学label，辅助塔学label+teacher logit
    + 蒸馏loss设计：mse比较不稳定，交叉熵更好；loss_gt + alpha * loss_distill，不强制权重和为1，以gt为主，调distill的scale
    + 采样纠偏：teacher和student的采样逻辑不一致，解法是teacher落原始logit，蒸馏时，按student采样纠偏后的新logit当成student要学的目标

![](../assets/rec-distill-arch.png)

核心是teacher先消费，边训边把logit落到存储里，并往现有样本里填充，放到新的样本流给student delay训练，流式数据也会定时dump到集群变成批式数据，供后续其他student迭代用
...
[我用 DeepSeek 网页版拿下 KDD Cup 冠军！](https://zhuanlan.zhihu.com/p/2070276269984446381)

![](../assets/queryformer.png)

+ 输入侧：id类特征和dense特征使用不同的tokenizer，每个token有多个emb table（图中的H scaling），然后merge一下给后面的结构用
+ queryformer：下面的3个结构，堆叠多层：
    + CoTransformerBlock：用户侧u tokens做self-attn、广告侧i tokens做self-attn，两者之间分别cross-attn，产出新的u tokens和i tokens
    + QuerySeqCrossAttn：上面的i tokens当成query，和行为序列算cross-attn，得到query1
    + SeqQueryCrossAttn：q是序列tokens的pooling结果，kv是前面的u tokens和i tokens再加上query1，算cross-attn，得到增强的query2，和更新的u tokens和i tokens

### 亚军：UniDot

[UniDot: A Unified Network for Sequence Modeling and Feature Interaction in Large-scale Recommendation](https://arxiv.org/pdf/2608.16797)
...
+ 还把多个个性化特征(公式里的f)加权，直接加到q,d的cosine上：$$S(q, d)=w_0\left\langle e_q, e_d\right\rangle+\sum_{i=1}^n w_i f_i(q, d),$$，然后这个score同时学相关性label和engagement label：$$\mathcal{L}_{\mathrm{RAR}}=\lambda \mathcal{L}_{\mathrm{BCE}}\left(S, L_R\right)+(1-\lambda) \mathcal{L}_{\mathrm{BCE}}\left(S, L_E\right)$$
...
![](../assets/lightretriever-dense.png)
...
![](../assets/lightretriever-sparse.png)
...
![](../assets/self-evolving-recsys.png)

fast内层循环(offline agent)：输入当前排序模型配置和效果给llm，走think-code-verify循环，自己改代码（layer、优化器、loss、label等），然后在离线数据集上训练模型并重新改代码。有如下3类角色

+ Optimizer：使用工具compute_loss，迭代优化器、超参（bs/动量等），看不同配置下训出来模型的loss差异
+ Architecture：使用工具compute_loss，迭代网络结构
+ Reward：使用工具run_sql_query，自己分析各label和特征的分布，设计更好的label
...
![](../assets/self-evolving-recsys-prompt.png)

![](../assets/self-evolving-recsys-modelsize.png)
...
![](../assets/actor-critic-prompt.png)

## Google的RecEvolve

[RecEvolve: A Knowledge-Driven Autonomous Agent System forRecommender Systems](https://arxiv.org/pdf/2609.01622)

![](../assets/auto-research-lifecycle.png)

+ hypothesis formulation: 分析现有模型瓶颈，并给出模型结构修改建议
+ pre-excecution validation: 类似质量gate，在真正提交资源前验证计划的可行性
+ automated implementation: 在独立分支里自动实现相关idea的代码
+ empirical evaluation & synthesis: 模型训练，并合成结论，更新中心化的知识库

![](../assets/knowledge-driven-orchestration.png)

多agent编排框架：有一个中心化的orchestrator，分配任务给specialized的subagents（ideator, critic, coding agent），

![](../assets/recevolve.png)
...
[Decoding ML Decision: An Agentic Reasoning Framework for Large-Scale Ranking System](https://arxiv.org/pdf/2602.18640)

GEARS (Generative Engine for Agentic Ranking Systems)

![](../assets/gears.png)

![](../assets/gears-algo.png)

![](../assets/gears-2.png)

## Meta的CORAL

[CORAL: An LLM-Native Harness for Production Recommender Systems](https://arxiv.org/pdf/2609.02730)

![](../assets/coral-harness.png)

## Meta的A-MLE

[Agentic ML Exploration (A-MLE) for Ads Ranking](https://arxiv.org/pdf/2609.08248v1)
...
[EvoRec: Self-Evolving Agentic Recommender Systems](https://arxiv.org/pdf/2606.28368)

## autoresearch总结

[AutoResearch怎么设计？四种常见循环，一套通用框架](https://mp.weixin.qq.com/s/nOfZsQobBh1MK7_7ZarEwg)

### 线性循环 Keep-or-Discard

代表系统：Karpathy [autoresearch](https://github.com/karpathy/autoresearch)（2025），每次尝试一个想法，如果结果更好就保留，否则回退。

![](../assets/keep-or-discard.png)
...
代表系统：[AIDE](https://arxiv.org/abs/2403.17373)（2024）、[AI Scientist v2](https://github.com/SakanaAI/AI-Scientist-v2)（2025）

### 遗传进化池循环

代表系统：[FunSearch](https://storage.googleapis.com/deepmind-media/DeepMind.com/Blog/funsearch-making-new-discoveries-in-mathematical-sciences-using-large-language-models/Mathematical-discoveries-from-program-search-with-large-language-models.pdf)（2023）、[AlphaEvolve](https://arxiv.org/abs/2506.13131)（2025）、[GEPA](https://arxiv.org/abs/2507.19457)（2025）
...
代表系统：[CORAL](https://arxiv.org/abs/2604.01658)（2026）

![](../assets/coral.png)

使用多个Agent各自独立运行完整的搜索循环，通过共享持久记忆间接协调，无需任何显式通信协议。

共享持久记忆以文件系统的形式实现，分为三个目录：attempts/存储所有历史评估记录（JSON 格式，按 commit hash 索引）、notes/存储观察和反思（Markdown 格式，支持合并和分类）、skills/存储可复用的过程和工具（包含自然语言描述和可执行脚本）。
...
[Heterogeneous Ranking in Industrial-Scale Recommender Systems: A Case Study](https://arxiv.org/pdf/2607.27577)

发现频道里有Web文章、长视频、短视频、UGC、用户关注的创作者内容、商品内容、AI摘要卡片等多体裁，如何在同一个模型里一起建模，并缓解跷跷板和多数类型偏置的问题
...
+ ha-gating：输入原ranking特征+h，得到一个gating值
+ HDLM（Heterogeneity-Driven Linear Modulation）：每个expert的输出会用h再做一下线性缩放和平移：$$\tilde{E}_n(x, h)=\gamma_n(h) \odot E_n(x)+\beta_n(h)$$

最终loss是pointwise的bce加上pairwise的ranknet(同一用户session里的正负反馈)

还搞了个DL-AUC，，lambda设置成0.8
...
+ Micro-AUC：衡量整体排序效果
+ Macro-xAUC：对不同内容类型之间的正负样本排序分别计算xAUC(cross auc)，再等权平均（详见原论文）

LENS(Latent Expert Network Specialization)：监控专家是否真正分工

+ Activation Slicing：将gate激活按“任务x内容类型x专家”切片并归一化，观察：
    + 通用任务是否合理共享专家
    + 格式相关任务是否形成内容类型特定的专家使用模式
    + 是否发生单专家垄断
+ Permutation-Invariant Expert Matching (PIEM)：不同训练轮次中，专家编号可以任意交换，所以不能直接比较“专家 1”和“专家 1”。PIEM是诊断信号，不是效果指标。 独立训练得到不同专家分工不一定意味着模型变差，但单专家坍缩通常会产生明显异常。
    + 用每个专家在不同任务上的贡献构造行为分布；
    + 用 Jensen–Shannon Divergence 计算专家差异；
    + 用匈牙利算法找到两次训练之间的最优专家匹配；
    + 得到忽略专家编号置换后的结构相似度。
...
$$
c^f=\sum_1^{L-1} c_l^f *\left(\Pi_{k=1}^{L-l} N_k\right)+c_L^f,
$$
...
![](../assets/mfli-rebalancing.png)

![](../assets/mfli-index-structure.png)
...
![](../assets/meta-adaptive-ranking-model.png)

![](../assets/meta-adaptive-ranking-model2.png)
...
![](../assets/store-1epoch.png)

有个疑问，这是个啥场景，gauc低可以理解，但auc咋也0.6几？

## 阿里的EST

[EST: Towards Efficient Scaling Laws in Click-Through Rate Prediction via Unified Modeling](https://arxiv.org/pdf/2602.10811)

里面有个热启的multi-epoch的范式，sparse一直load最开始那个sparse，dense每轮用最新的

参考的是[Multi-Epoch Learning for Deep Click-Through Rate Prediction Models](https://arxiv.org/pdf/2305.19531)和[Multi-Epoch learning with Data Augmentation for Deep Click-Through Rate Prediction](https://arxiv.org/pdf/2407.01607)

![](../assets/est-multi-epoch.png)
...
[https://github.com/Kaike-Zhang/GoalRank](https://github.com/Kaike-Zhang/GoalRank)
...
本文将两条原本相对独立的工业推荐研究路线放到同一分析框架中：

- Dense feature interaction 与 ranking model scaling：Wukong、RankMixer 及其后续。
- 长用户行为序列建模：HSTU、LONGER、STCA、SlimPer 及其后续。

目标不是逐篇复述，而是回答三个架构问题：

1. 计算预算应该用于异构特征交互，还是用于行为序列推理？
2. 序列特征与非序列特征应该在模型的哪个阶段融合？
3. 当一个请求包含大量候选 item 时，哪些用户侧计算可以复用？

整理范围以工业推荐、广告和搜索排序论文为主，信息更新至 2026 年 8 月。

> 名称提醒：[Hiformer](https://arxiv.org/abs/2311.05884) 是 Google 的异构特征交互模型；[HyFormer](https://arxiv.org/abs/2601.12681) 是 ByteDance 的序列与特征统一模型。两者不是同一篇工作。
...
```text
Dense Scaling:
DHEN / Hiformer / Wukong
        -> RankMixer
        -> TokenMixer-Large / UniMixer / RankElastor / RankUp
Long Sequence:
DIN / SIM / TWIN
        -> HSTU / LONGER / STCA
        -> SlimPer / VISTA / ULTRA-HSTU
Unified Modeling:
InterFormer
        -> OneTrans / HyFormer / MixFormer / EST / INFNet / WHALE
```

最重要的判断如下。

- RankMixer 主要解决固定数量异构 feature tokens 的交互和 dense scaling，不直接解决长序列。
- LONGER、STCA、SlimPer 主要解决如何让少量 Query 高效读取长历史。
- 工业强基线常采用 `Sequence Encoder -> RankMixer`，但这是一种 late-fusion 架构。
- Late fusion 的主要损失不是“少了一次 concat”，而是 RankMixer 学到的高阶特征无法反馈给序列读取过程。
- HyFormer 和 MixFormer 的核心贡献，是把 RankMixer 式特征交互插入每一层历史读取。
- SlimPer、HyFormer、MixFormer 都属于 compact-query 路线，但 Query 的语义不同。
- HSTU、MTGR、WHALE 保留 history-to-history self-attention，质量上限通常更高，成本也更高。
- OneTrans、EST、TokenFormer 试图保留原始 token 流，同时通过 Query 缩减或稀疏 attention 控制成本。
- INFNet 使用 latent hubs 聚合后再广播回原始 tokens，是 compact bottleneck 与完整 token propagation 的折中。
- 统一模型最终都会遇到同一个系统问题：user/item 过早混合会破坏多候选请求中的用户侧复用。

可以按照“层间保留多少历史状态”理解当前主流架构：

| 类型 | 代表模型 | 层间保留的状态 | 行为之间的交互 | 主要取舍 |
|---|---|---|---|---|
| Compact state | STCA、SlimPer、HyFormer、MixFormer | 少量 target、knowledge slot 或 feature-query tokens | 不直接维护完整 history-to-history 交互 | 激活内存低，重点建模候选与历史的相关性 |
| Compact hubs + full tokens | INFNet | 少量 hub 和完整原始 tokens | hub 聚合后将信息广播回原始 tokens | 保留细粒度状态，但内存随历史长度增长 |
| Sparse or shrinking token stream | EST、OneTrans | 完整或逐层缩短的 token 流 | 使用稀疏 attention，或逐层减少 Query | 在计算成本和序列推理能力之间折中 |
| Full sequence state | HSTU | 每层完整历史 token 流 | 完整 causal history-to-history self-attention | 序列推理能力强，但计算和激活成本最高 |

从左到右并不是严格的模型优劣排序，而是计算分配方式的变化：

```text
更多计算用于候选相关性
    -> 更多计算用于维护细粒度历史状态
    -> 更多计算用于历史 token 之间的序列推理
```
...
```text
L: 用户历史长度
M: 非序列 feature token 数
q: compact query 数
K: latent / knowledge slot 数
D: hidden dimension
S: 模型层数
C: 同一请求中的候选 item 数
```

与其按公司或年份分类，更有效的方式是观察三个维度。

**序列状态保留程度**

第一类只传播 compact state：

```text
Q_(l+1) = CrossAttention(Q_l, H, H)
```
...
它们不会在每层维护长度为 `L` 的历史 hidden states，激活内存主要由 `q`、`K` 或 `M` 决定。

第二类传播完整或近完整 token state：

```text
H_(l+1) = SelfAttention(H_l, H_l, H_l)
```
...
这类模型能够直接建模行为之间的顺序、共现和高阶依赖，但每层成本通常与 `L^2` 或稀疏 attention graph 相关。

第三类使用持久化摘要或 memory：

```text
Long History -> Offline/Incremental Summary -> Online Ranking
```
...
它们把长历史计算移到离线或 nearline 阶段，在线成本最低，但候选相关性通常弱于端到端方法。

**序列与 Dense 特征的融合深度**

```text
Late Fusion:
HistoryEncoder(H) -> summary
FeatureMixer([summary; F])
Alternating Fusion:
Q_l reads H
Q_l mixes with F
repeat
Monolithic Stream:
[H; F] enters one token stream
Dual Branch:
Sequence branch and dense branch remain active
fusion happens at every layer
```
...
| 融合方式 | 代表模型 | 主要特征 |
|---|---|---|
| Late Fusion | LONGER -> RankMixer、STCA -> RankMixer | 模块成熟，但信息流单向 |
| Alternating Fusion | HyFormer、MixFormer、SlimPer | compact Query 反复读取历史 |
| Monolithic Stream | OneTrans、TokenFormer、EST、MTGR | 交互最直接，但计算图和复用更难 |
| Dual Branch | InterFormer、WHALE | 保留专用算子，融合成本较高 |
| Hub Aggregate/Broadcast | INFNet | 线性复杂度下保留完整 token state |

**Request-Level Reuse**

对同一用户请求中的 `C` 个候选，如果用户侧表示依赖候选 item，则计算可能重复 `C` 次。

高效架构通常约束信息流：

```text
user state 不能读取 item-specific signal
item state 可以读取 user state
```

代表设计：

- HSTU 的 M-FALCON。
- LONGER 和 OneTrans 的 KV cache。
- STCA 的 Request Level Batching。
- SlimPer 的 Request-Only Optimization。
- UG-Sep 的 user-group separation。
- UI-MixFormer 的 masked HeadMixing。
- EST 的 user-candidate decoupled computation。

这说明“更深融合”和“更强复用”天然存在张力。

## 3. Dense Scaling 与 RankMixer

Dense scaling 关注的是：

> 在固定 feature token 数量下，如何通过增加模型深度、宽度和参数量，持续提升异构特征交互能力？

早期代表工作如下。

| 论文 | 公司 | 交互算子 | 主要意义 |
|---|---|---|---|
| [DHEN](https://arxiv.org/abs/2203.11014) | Meta | DCN、self-attention、FM 等层次 ensemble | 证明统一 interaction block 可以堆叠扩展 |
| [Hiformer](https://arxiv.org/abs/2311.05884) | Google | token-specific heterogeneous attention | 用独立 Q/K/V 处理异构 feature spaces |
| [Wukong](https://arxiv.org/abs/2403.02545) | Meta | Factorization Machine Block + Linear Compression | 建立 feature-interaction dense scaling law |
| [RankMixer](https://arxiv.org/abs/2507.15551) | ByteDance | parameter-free Token Mixing + Per-token FFN | 将参数和 FLOPs 放在 GPU 友好的 FFN 中 |

**RankMixer 的本质**

输入为固定数量的语义 feature tokens：

```text
X = [x_1, x_2, ..., x_M] in R^(M x D)
```

每个 token 沿 hidden dimension 切成多个 head：

```text
x_i = [x_i^1 | x_i^2 | ... | x_i^M]
```

再将所有 token 的同一 head 拼接成新 token：

```text
s_j = Concat(x_1^j, x_2^j, ..., x_M^j)
```

这个操作本质上是固定 reshape 和 transpose：

```text
TokenMix(X) = Reshape(Transpose(Reshape(X)))
```

随后每个 mixed token 使用独立 FFN：
...
```text
异构 feature-to-feature 交互:
不用 similarity-based self-attention
参数容量:
主要放入 token-specific FFN
```

原因是 user ID、item ID、context、统计特征来自不同语义空间，向量内积未必能表示可靠相似度。

RankMixer 的优势：

- 不构造 feature-token attention score matrix。
- TokenMix 无参数，计算和通信规则稳定。
- PFFN 能隔离不同 feature subspace 的参数。
- 大矩阵计算适合 GPU，论文报告 MFU 显著提升。
- 易于扩展 Dense MoE 或 Sparse MoE。

RankMixer 的局限：

- Mixing 是固定 permutation，不随样本学习。
- Mixing 前后 token 的语义位置可能不一致，影响 residual。
- 深层网络存在梯度和训练稳定性问题。
- TokenMix 扩展有效秩，但 PFFN 可能再次压缩有效秩。
- 它只接收序列 summary，无法直接利用细粒度长历史。
- user/item token 过早混合后，不利于用户侧请求级复用。

**RankMixer 的直接演化**

| 论文 | 解决的问题 | 关键改动 | 架构含义 |
|---|---|---|---|
| [TokenMixer-Large](https://arxiv.org/abs/2602.06563) | 深度、residual、MoE 和超大规模训练 | Mixing-and-Reverting、跨层残差、辅助 loss、Sparse-Pertoken MoE、FP8 | RankMixer 最直接的工业升级 |
| [UG-Sep](https://arxiv.org/abs/2602.10455) | 多候选重复计算用户侧 token | Masked information flow、可复用 PertokenAFFN、信息补偿 | 将 dense backbone 改造成 request-aware |
| [UniMixer](https://arxiv.org/abs/2604.00590) | 固定 TokenMix 不可学习 | 将 permutation matrix 推广为结构化可学习 mixing | 统一 attention、TokenMixer 和 FM |
| [RankElastor](https://arxiv.org/abs/2605.23191) | TokenMix 扩秩不足、PFFN 秩收缩 | Parameterized Full Mixing、GLU PFFN | 从有效秩角度修复深层 scaling |
| [RankUp](https://arxiv.org/abs/2604.17878) | 深层表示发生 collapse | 随机拆分、多 embedding、global/task/pretrained tokens | 从 token 组织和输入多样性提升秩 |

这些后续工作揭示了一个共同事实：

> 参数量扩大并不等于有效表示容量扩大。

TokenMixer-Large 更偏工程和深层训练；UniMixer 更偏 mixing 算子统一；RankElastor 和 RankUp 更关注表示谱与有效秩；UG-Sep 解决在线系统复用。

因此，RankMixer 路线已经从“如何堆大模型”发展为四个子问题：

```text
可学习 mixing
深层训练稳定性
有效秩与 representation collapse
请求级计算复用
```
...
> 给定 `10^3` 到 `10^6` 条用户行为，如何在工业延迟和内存约束下提取候选相关兴趣？

主要有四类策略。

| 路线 | 代表论文 | 长历史处理 | 在线中间状态 |
|---|---|---|---|
| 检索后精排 | [SIM](https://arxiv.org/abs/2006.05639)、[ETA](https://arxiv.org/abs/2209.12212)、[TWIN](https://arxiv.org/abs/2302.02352) | 从完整历史检索 Top-K，再做 target attention | Top-K 子序列 |
| Offline/Memory | [MIMN](https://arxiv.org/abs/1905.09248)、[DV365](https://arxiv.org/abs/2506.00450)、[VISTA](https://arxiv.org/abs/2510.22049) | 增量或离线压缩成 memory/summary tokens | 固定 memory |
| Compact Query | [LONGER](https://arxiv.org/abs/2505.04421)、[STCA](https://arxiv.org/abs/2511.06077)、[SlimPer](https://arxiv.org/abs/2607.12281) | 少量 Query 读取长 K/V | 短 Query 或 knowledge slots |
| Full/Near-Full SA | [HSTU](https://arxiv.org/abs/2402.17152)、[MTGR](https://arxiv.org/abs/2505.18654)、[ULTRA-HSTU](https://arxiv.org/abs/2602.16986) | 在完整或 semi-local token graph 上做序列推理 | 长度随历史增长 |

检索路线可以扩展到极长历史，但未召回 token 无法参与后续推理。

Memory 路线在线成本最低，但摘要往往是 item-independent，可能损失候选相关细节。

Compact-query 路线保留完整历史作为只读证据库，重点计算 Query-to-History relevance。

Full self-attention 路线保留完整 token-to-token reasoning，质量潜力高，但计算和激活成本最大。
...
```text
Q = [global tokens; sampled history], length m+k
K,V = [global tokens; full/merged history], length m+L
Z_1 = CrossAttention(Q, K, V)
```

后续层：

```text
Z_(l+1) = SelfAttention(Z_l)
```

因此它不是在完整 `L` 上持续做 self-attention，而是在第一层输出的 `m+k` 长度上做。
...
每层 target state 都重新读取完整历史，不维护 history hidden states。

SlimPer：

```text
Q_l = Project(KnowledgeBase_l)
R_l = CrossAttention(Q_l, all user-side tokens)
KnowledgeBase_(l+1) = MatchAndRefine(KnowledgeBase_l, R_l)
```

它将 target state 扩展成固定大小 user-item knowledge base，并加入显式 matching/refinement。

HSTU：

```text
H_(l+1) = GatedCausalSelfAttention(H_l)
```

它在完整输入窗口上保留 history-to-history reasoning。原始 HSTU 为二次复杂度，ULTRA-HSTU 后续转向 semi-local attention。

这四个模型形成清晰取舍：

| 模型 | Query 长度 | 每层重读原始历史 | history-to-history SA | 层间状态 |
|---|---:|---|---|---|
| LONGER | `m+k`，可随历史增长 | 主要在首层 | 在压缩长度上有 | sampled/global tokens |
| STCA | 很小 | 是 | 无 | target state |
| SlimPer | 固定 `q` / `K` | 是 | 无 | knowledge base |
| HSTU | 约等于输入窗口 | 不适用 | 有 | 完整 token stream |

真正的争论不是“有没有 attention”，而是：

> 是否值得为 history-to-history 交互支付接近 `O(L^2)` 的成本？

支持 full self-attention 的理由：

- 行为间存在顺序、重复、周期和高阶依赖。
- 单纯 target-to-history attention 不能先完成复杂的历史内部推理。
- HSTU 和 ULTRA-HSTU 认为 self-attention 仍有更高质量上限。

支持 compact-query 的理由：

- 排序最终只需要少量 user-item relevance 表示。
- 大量 history-to-history 计算与当前候选无关。
- 深度与历史激活大小可以解耦。
- 用户侧 K/V 更容易在候选之间共享。
- 10k 以上历史更容易满足生产延迟。
...
```text
H -> LONGER/STCA/HSTU -> sequence summary
[sequence summary; user/item/context/cross features]
    -> RankMixer/Wukong/DCN
    -> prediction
```

这种结构的问题不是模块能力不足，而是信息流方向受限：

```text
history -> summary -> dense interaction
dense interaction 无法反馈到 history retrieval
```

统一模型试图让两类信号在更深层反复交换。

| 模型 | 基本结构 | 序列状态 | Dense 交互方式 | 融合位置 |
|---|---|---|---|---|
| [InterFormer](https://arxiv.org/abs/2411.09852) | Interaction/Sequence 双分支 + Cross Arch | 独立 sequence branch | 独立 interaction branch | 每层双向交换 |
| [HyFormer](https://arxiv.org/abs/2601.12681) | Query Decoding + Query Boosting | compact global queries | RankMixer/MLP-Mixer 风格 | 每层交替 |
| [MixFormer](https://arxiv.org/abs/2602.14110) | Query Mixer + Cross Attention + Fusion | feature-conditioned heads | RankMixer HeadMixing + PFFN | 单 block 内统一 |
| [OneTrans](https://arxiv.org/abs/2510.26104) | 单一 causal Transformer | Query 逐层缩短 | mixed-parameter self-attention | 单流每层 |
| [EST](https://arxiv.org/abs/2602.10811) | LCA + Content Sparse Attention | 完整原始 tokens | token-specific interaction | 稀疏统一 graph |
| [INFNet](https://arxiv.org/abs/2508.11565) | Hub Aggregate + Broadcast | 完整 tokens 被反复更新 | hub-mediated interaction | 每层双向 |
| [WHALE](https://arxiv.org/abs/2607.17017) | Wukong + HSTU + Fusion | 完整 HSTU branch | Wukong FM branch | 每层跨分支 |

**HyFormer 与 MixFormer**

HyFormer 每层先读取历史，再增强 Query：
...
```text
历史 -> Query -> 异构特征交互 -> 更强 Query -> 再读历史
```

其优势是多序列组织清晰：

```text
watch sequence    <-> watch query
search sequence   <-> search query
purchase sequence <-> purchase query
```

不同序列不必强制拼接，跨序列交互发生在 Query Boosting 阶段。

MixFormer 每层先生成高阶特征 Query，再读取历史：

```text
Q_l = QueryMixer(F_l)
Z_l = CrossAttention(Q_l, K_l(H), V_l(H))
...
```text
高阶异构特征 -> 专门 Query heads -> 读取历史 -> 融合 -> 重复
```

Query Mixer 使用 RankMixer 式 HeadMixing 和 per-head SwiGLU，不用 self-attention 比较异构 feature tokens。

Cross Attention 仍然保留，因为 structured Query 与历史行为之间的相关性具有明确语义。

两者的差别：

| 维度 | HyFormer | MixFormer |
|---|---|---|
| 每层顺序 | 先 Cross-Attention，后 Query Boost | 先 Query Mixer，后 Cross-Attention |
| Query 组织 | 每条序列可有独立 global query | 多个高阶 feature heads |
| Dense 参数 | Query Boost 中的 token-specific FFN | Query/Fusion 中共享的 per-head FFN |
| 主要目标 | 让异构信息持续增强下一层 Query | 让 dense 与 sequence 共享可 co-scale 参数 |
| 多序列优势 | 明确且灵活 | 论文重点更偏统一 head |
| 请求级复用 | 可结合 KV cache/M-FALCON | UI-MixFormer 明确加入 mask |

它们都可以抽象为：

```text
Q_l = DenseInteraction(Q_l, F)
Q_l = CrossAttention(Q_l, H, H)
```

HyFormer 与 MixFormer 相比 SlimPer：

- 三者都只传播 compact user-item state。
- 三者都在多层中重新读取长历史。
- SlimPer 用显式 knowledge slots、matching 和 refinement。
- HyFormer 用 Query Boosting 建模跨 Query、跨序列、非序列特征交互。
- MixFormer 让 RankMixer-style heads 同时承担 dense interaction 和 sequence Query。

其他统一路线代表不同折中。

OneTrans 将所有 token 放入统一 causal stream，并逐层减少发出 Query 的 sequence tokens。

EST 保留原始 token，但删除低价值 self-interaction，只保留 non-behavior-to-behavior cross edges 和内容相似 top-K 行为边。

INFNet 用少量 hubs 聚合全局信息，再将信息广播回完整 token 流，因此避免 SlimPer 式只传播 compact state。

WHALE 不折叠两种 backbone，而是同时保留 Wukong 和 HSTU，并让 Wukong 表示逐层查询 HSTU 序列表示。

## 6. 总体对比与架构选择

**统一复杂度和状态对比**

| 模型 | 长历史读取 | History SA | Dense 交互 | 层间状态 | 典型序列成本 | Request Reuse |
|---|---|---|---|---|---|---|
| RankMixer | 不直接读原始历史 | 无 | TokenMix + PFFN | `O(MD)` | 与 `L` 无关 | 原始版较弱 |
| LONGER -> RankMixer | 首层短 Q 读长 KV | 压缩长度上有 | RankMixer | `O(kD+MD)` | `O(kL+k^2)` | KV cache |
| STCA -> RankMixer | target Q 每层读历史 | 无 | late RankMixer | 固定 | `O(SqLD)` | RLB |
| SlimPer | slots 每层读全部用户 token | 无 | Match + Refine | `O(KD)` | `O(SqLD)` | ROO |
| HyFormer | global Q 每层读各自序列 | 可配置 | Query Boost | `O((q+M)D)` | `O(SqLD)` | 可结合 cache |
| MixFormer | feature heads 每层读历史 | 无 | Query Mixer + Fusion | `O(MD)` | `O(SMLD)` | UI mask |
| OneTrans | 统一 causal stream | 浅层有 | Self-Attention | 逐层缩短 | 介于 full 与 cross | KV cache |
| EST | 原始 token + 稀疏 graph | top-K sparse | LCA | `O(LD)` | 近似线性 | 用户侧可复用 |
| INFNet | hubs 聚合后广播 | 无 dense SA | Hub flow | `O(LD)` | 线性 | 取决于 token 分组 |
| HSTU/MTGR | 完整窗口推理 | 有 | Self-Attention | `O(LD)` | `O(SL^2D)` | M-FALCON |
| WHALE | HSTU branch | 有 | Wukong branch | 双分支 | HSTU + Wukong | 系统优化 |

表中的 Big-O 省略了 projection、FFN、head 数和 batch 相关常数，适合判断扩展趋势，不适合直接预测线上延迟。

**如何选择**

如果增益主要来自丰富 sparse、cross 和 context features：
...
- 序列长度较稳定。
- 已有成熟 sequence summary。
- GPU MFU 和 dense 参数扩展是主要瓶颈。
...
如果 dense 与 sequence 都重要，并且 late fusion 已成为瓶颈：

```text
多序列、Query 组织优先:
HyFormer
Dense/Sequence co-scaling 优先:
MixFormer
统一 token stream:
OneTrans / EST
保留两套成熟 backbone:
...
如果不希望丢弃原始 token state，但 full self-attention 太贵：

```text
结构化稀疏:
EST
Latent aggregate + full-token broadcast:
INFNet
```
...
必须优先检查:
用户侧状态是否依赖 candidate
哪些 K/V、FFN 和 tokenization 可以 request-level shared
```

此时 UG-Sep、UI-MixFormer、ROO、RLB、M-FALCON 的系统价值可能高于单纯扩大模型。

**三个研究判断**

第一，feature interaction 和 sequence reasoning 需要不同归纳偏置。

```text
Feature-to-Feature:
异构语义空间，固定 mixing、token-specific FFN、FM 往往更合适
Query-to-History:
相关性检索语义明确，cross-attention 往往更合适
History-to-History:
顺序和高阶依赖重要，self-attention 更强但更贵
```

第二，统一架构的价值主要来自信息流，而不是“共享一个 Transformer 名字”。

真正有效的统一应允许：

```text
dense feature interaction 改变下一层 sequence query
sequence evidence 反过来改变下一层 dense representation
```

如果只是把所有输出 concat 到一起，仍然属于 late fusion。

第三，生产架构不能只看单候选 FLOPs。
...
用户侧 token 是否复制
K/V 是否可缓存
通信和 embedding lookup
激活内存
算子 MFU
稀疏路由负载均衡
```

这也是为什么 SlimPer、UG-Sep、UI-MixFormer 和 HSTU 系统优化都强调请求级计算图。

## 7. 阅读路线与论文索引

如果只读十篇，推荐顺序如下。

1. [Wukong](https://arxiv.org/abs/2403.02545)：理解 dense feature-interaction scaling。
2. [RankMixer](https://arxiv.org/abs/2507.15551)：理解 TokenMix + PFFN 和 GPU-aware 设计。
3. [TokenMixer-Large](https://arxiv.org/abs/2602.06563)：理解 RankMixer 的深层训练和 MoE 升级。
4. [HSTU](https://arxiv.org/abs/2402.17152)：理解 full sequence reasoning 路线。
5. [LONGER](https://arxiv.org/abs/2505.04421)：理解短 Query 读取长 K/V 后继续短序列建模。
6. [STCA](https://arxiv.org/abs/2511.06077)：理解 stacked target-to-history cross-attention。
7. [SlimPer](https://arxiv.org/abs/2607.12281)：理解固定 knowledge slots 和 recommendation-centric refinement。
8. [HyFormer](https://arxiv.org/abs/2601.12681)：理解 Query Decoding 与 Query Boosting。
9. [MixFormer](https://arxiv.org/abs/2602.14110)：理解 dense/sequence co-scaling。
10. [EST](https://arxiv.org/abs/2602.10811) 或 [WHALE](https://arxiv.org/abs/2607.17017)：比较稀疏统一与双分支统一。

按主题补充阅读：

| 主题 | 论文 |
|---|---|
| Lifelong 检索 | [SIM](https://arxiv.org/abs/2006.05639)、[ETA](https://arxiv.org/abs/2209.12212)、[TWIN](https://arxiv.org/abs/2302.02352)、[TWIN-V2](https://arxiv.org/abs/2407.16357) |
| Offline/Memory | [MIMN](https://arxiv.org/abs/1905.09248)、[MARM](https://arxiv.org/abs/2411.09425)、[DV365](https://arxiv.org/abs/2506.00450)、[VISTA](https://arxiv.org/abs/2510.22049) |
| Full/Near-Full SA | [HSTU](https://arxiv.org/abs/2402.17152)、[MTGR](https://arxiv.org/abs/2505.18654)、[GRAB](https://arxiv.org/abs/2602.01865)、[ULTRA-HSTU](https://arxiv.org/abs/2602.16986) |
| RankMixer 后续 | [UG-Sep](https://arxiv.org/abs/2602.10455)、[UniMixer](https://arxiv.org/abs/2604.00590)、[RankElastor](https://arxiv.org/abs/2605.23191)、[RankUp](https://arxiv.org/abs/2604.17878) |
| 统一建模 | [InterFormer](https://arxiv.org/abs/2411.09852)、[OneTrans](https://arxiv.org/abs/2510.26104)、[TokenFormer](https://arxiv.org/abs/2604.13737)、[INFNet](https://arxiv.org/abs/2508.11565)、[EST](https://arxiv.org/abs/2602.10811)、[WHALE](https://arxiv.org/abs/2607.17017) |

最终可以用一句话概括整个技术演化：

> 工业推荐架构正在从“一个序列 encoder 加一个 feature interaction head”，演化为“用异构高阶特征持续构造 Query、分层读取长历史，并在多候选请求中显式复用用户侧计算”的统一模型。
