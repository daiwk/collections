...
[近500页史上最全扩散模型修炼宝典，宋飏等人一书覆盖三大主流视角](https://mp.weixin.qq.com/s/tBVkAK3TvlEBplpCYRtz1A)

[The Principles of Diffusion Models: From Origins to Advances](https://arxiv.org/pdf/2510.21890)
...
![stable-diffusion](<.gitbook/assets/stable-diffusion (1).png>)
...
粗略看了下代码，带condition的训练原理大概是训练语料中有图+文本（例如imagenet的class\_label，这里可以映射到一个classid也可以直接拿明文），然后condition和图片一起作为输入去训练auto-eocnder和ldm

在`/latent-diffusion/ldm/data/imagenet.py`这个代码里，把class\_label加进来了
...
![tarsier](<.gitbook/assets/tarsier (1).png>)
...
![bsq-vq-lfq](<.gitbook/assets/bsq-vq-lfq (1).png>)

![bsq-fsq-lfq](<.gitbook/assets/bsq-fsq-lfq (1).png>)
...
* SAM：[CV不存在了？Meta发布「分割一切」AI 模型，CV或迎来GPT-3时刻](https://mp.weixin.qq.com/s?__biz=MzA3MzI4MjgzMw==\&mid=2650872938\&idx=2\&sn=ddb938b9c7e6206a0b97032cb9dee30a\&scene=21)，[Segment Anything](https://arxiv.org/abs/2304.02643)
* SAM2：[刚刚，Meta开源「分割一切」2.0模型，视频也能分割了](https://mp.weixin.qq.com/s?__biz=MzA3MzI4MjgzMw==\&mid=2650928134\&idx=1\&sn=7855c453083236c5c5d121d3b4fb955e\&scene=21)，[SAM 2: Segment Anything in Images and Videos](https://arxiv.org/abs/2408.00714)
* SAM3：[ICLR 2026惊现SAM 3，分割一切的下一步：让模型理解「概念」](https://mp.weixin.qq.com/s/7uDHXQd1ES2mV4dZFB7VMw)，[SAM 3: Segment Anything with Concepts](https://openreview.net/pdf?id=r35clVtGzw)

可以看这个demo，[https://github.com/facebookresearch/segment-anything/blob/main/notebooks/predictor\_example.ipynb](https://github.com/facebookresearch/segment-anything/blob/main/notebooks/predictor_example.ipynb)，输入的可以是一个或者多个点（以及每个点是0还是1，即是否mask），也可以是一个或者多个box。例如输入的点在车窗里，那输出的mask就是当前这个车窗（置信度最高），多个车窗（置信度次之）、整辆车（置信度更低）；例如输入两个点，一个在车窗里（label是1），一个是车门(label是0)，那输出的mask就是整个车窗，没有车门。

### RAE

[VAE时代终结？谢赛宁团队「RAE」登场，表征自编码器或成DiT训练新基石](https://mp.weixin.qq.com/s/Z1ZJnco6oEXWMUdbrito9w)

[Diffusion Transformers with Representation Autoencoders](https://arxiv.org/abs/2510.11690)

[https://github.com/bytetriper/RAE](https://github.com/bytetriper/RAE)
...
![](<.gitbook/assets/rae (1).png>)

原来的SD-VAE把$$256\times 256$$的图像压缩到$$32\times 32\times 4$$的潜在表征，限制了重建的保真度，也限制了表征的质量。

假设输入图片是$$3\times H\times W$$，patch大小为$$p_e$$，隐层大小为$$d$$，那就是把原图像转成$$N=HW/p_{e}^2$$个token，每个token有$$d$$个通道，先过一个预训练好的freeze的编码器，然后过DiT，输出再过一个解码器，patch大小为$$p_d$$，变回$$3 \times H \frac{p_d}{p_e} \times W \frac{p_d}{p_e}$$，一般设置$$p_d=p_e$$，让输出和原图分辨率匹配
...
\begin{gathered}
z=E(x), \hat{x}=D(z) \\
\mathcal{L}_{\text {rec }}(x)=\omega_L \operatorname{LPIPS}(\hat{x}, x)+\mathrm{L} 1(\hat{x}, x)+\omega_G \lambda \operatorname{GAN}(\hat{x}, x)
\end{gathered}
$$
...
![InstantID](<.gitbook/assets/InstantID (1).png>)
...
* autoregressive transformers
  * 2021年的[Videogpt: Video generation using vq-vae and transformers](https://github.com/daiwk/collections/blob/master/posts/\(https:/arxiv.org/pdf/2104.10157.pdf\))
...
![vivit](<.gitbook/assets/vivit (1).png>)

先分patch，再分别过时间的transformer（temporal transformer）和空间的transformer（spatial transformer）

![tubelet-embedding](<.gitbook/assets/tubelet-embedding (1).png>)
...
![spacetime-patches](<.gitbook/assets/spacetime-patches (1).png>)
...
v1版本：[https://github.com/hpcaitech/Open-Sora/blob/main/docs/report\_v1.md](https://github.com/hpcaitech/Open-Sora/blob/main/docs/report_v1.md)

v2版本：[https://github.com/hpcaitech/Open-Sora/blob/main/docs/report\_02.md](https://github.com/hpcaitech/Open-Sora/blob/main/docs/report_02.md)
...
![latte](<.gitbook/assets/latte (1).png>)
...
![stdit](<.gitbook/assets/stdit (1).png>)
...
![pixart-alpha-temperal](<.gitbook/assets/pixart-alpha-temperal (1).png>)
...
![llava-caption](<.gitbook/assets/llava-caption (1).png>)
...
模型地址：[https://huggingface.co/collections/YanweiLi/mini-gemini-6603c50b9b43d044171d0854](https://huggingface.co/collections/YanweiLi/mini-gemini-6603c50b9b43d044171d0854) 数据地址：[https://huggingface.co/collections/YanweiLi/mini-gemini-data-660463ea895a01d8f367624e](https://huggingface.co/collections/YanweiLi/mini-gemini-data-660463ea895a01d8f367624e)
...
![mini-gemini](<.gitbook/assets/mini-gemini (1).png>)
...
[谷歌Veo 3论文竟无一作者来自美国！揭秘零样本「看懂」世界](https://mp.weixin.qq.com/s/7XZNAlAH-CkF8_1gL0p5VA)

[Video models are zero-shot learners and reasoners](https://arxiv.org/abs/2509.20328)

..就是个评测而已，提到了chain-of-frames（CoF），相关的工作可以看[CoT 之后，CoF 如何让帧间逻辑从「隐式对齐」变成「显式思考」？](https://pro.jiqizhixin.com/reference/e1bd460e-a706-4965-a1bb-b3b072c89be4)
...
[阿里开源电影级AI视频模型！MoE架构，5B版本消费级显卡可跑](https://mp.weixin.qq.com/s/RKR16Qw1BdvlVSLMrj17lg?scene=1)
...
[跨境电商版Sora发布：全球首个AI原生电商视频Multi-Agent来了](https://mp.weixin.qq.com/s/-g6hRrvr1YyMiuZvwOLQcw)

![](.gitbook/assets/insail-hilight.png)

* 理解与洞察层：4个Agent接收到需求后，负责理解用户输入的需求和素材，像资深策划一样拆解需求、洞察「卖点」和受众，从而将模糊需求变成精准的营销指令。
* 创意与结构层：4个Agent负责「努力干活」，创意Agent写脚本，剧本策划Agent会拆成详细带有时间轴的分镜，紧接着素材匹配Agent进行素材匹配，以及定向优化/修复，目的就是将「好想法」变成能被执行的视频结构，将其交付给下一层级执行。
* 执行与成片层：剪辑执行Agent和成片生成Agent负责把所有的素材进行按轨道、时间排列成片。最后，质检复盘Agent还会对成片质量进行最终检验，以保证最终输出一个完整的、可投放的视频资产。

### 字节的Bernini

[字节开源统一框架Bernini：给DiT配个“大模型军师”，AI视频编辑先理解再动手](https://mp.weixin.qq.com/s/3CYSuFeN6Tk2eHFMsgjxVg)

[Bernini: Latent Semantic Planning for Video Diffusion](https://arxiv.org/pdf/2605.22344)
...
[Stream-Omni：同时支持各种模态组合交互的文本-视觉-语音多模态大模型](https://mp.weixin.qq.com/s/VFUvLoJsT7rOVjhCvLLmBA)
...
![](<.gitbook/assets/thinkdiff-lvlm+clip (1).png>)
...
[LLM为什么能看懂图？秘密不在Projector，残差结构才是跨模态理解的关键](https://mp.weixin.qq.com/s/B4SSNI2-0_zYsM4cLimNLQ)
...
![](<.gitbook/assets/cambrian-s-1 (1).png>)
