...
接下来讨论可以进行行交换的场景，即主元位置存在0的情况

置换（permutation）：从单位矩阵出发，对其行进行两两互换，可以得到$$A_n^n=n!$$个置换，它们的逆就在这些置换矩阵之中(其实就是它的转置)，它们的转置也在这些置换矩阵之中，它们的两两的乘积也在置换矩阵之中，这就构成了一个矩阵乘法运算下的群。。
