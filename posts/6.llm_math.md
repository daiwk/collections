...
考虑如下情况，之前学的消元，把第2行第1列消成0：

![](../assets/lu1.png)

然后左右各乘一个$$E_{21}$$的逆（第一行不变，第二行把-4变回4），这样就有如下的图的第一行，可以发现第一个是下三角矩阵（lower, L），第二个是上三角矩阵（upper, U）

![](../assets/lu2.png)

而第二行是对U矩阵进行变换，想加一个对角矩阵进来，那就可以算出第3个矩阵。第1个还是下三角，第2个是对角矩阵，第3个还是上三角

接下来看3x3的矩阵，假设没有行交换，需要进行3步：$$E_{32}E_{31}E_{21}A=U$$，所以$$A=E^{-1}_{21}E^{-1}_{31}E^{-1}_{32}U=LU$$，这里的$$L$$就是$$E_{32}E_{31}E_{21}$$这个乘积的逆

![](../assets/lu3.png)

可以发现，对于$$A=LU$$，如果没有行交换，那么$$L$$是下三角，对角线是1，下面的元素是消元所用的乘数，且$$U$$是上三角，主对角线是主元即下图

![](../assets/lu性质.png)

对于一个$$n \times n$$的矩阵，消元需要进行多少次操作？假设n=100

第一次变换是把第1行第1列(主元)下面的那一列全变成0，即第一行确定了，第一列确定了，剩下的是一个$$99\times 99$$的矩阵。这一次变换是对第i行乘一个数，然后被第2行减，假设乘+减算一次操作，这里对99行做了这个操作，所以就是$$99\times 100$$，大概是$$100^2$$的量级

第二次变换就是$$99^2$$的量级，以此类推，总共就是$$n^2+(n-1)^2+...+1^2$$，由于这里是递减的，大概是$$1/3n^3$$的量级。为啥是$$1/3$$？要用微积分，因为这个其实就是对$$y=x^2$$在x=1到n的曲线下方面积，可以拆成n个小长方形面积求和，即对$$x^2$$求积分就是$$1/3x^3$$。因为微积分就是对连续的情况下的求和，而线性代数是离散的

以上是对A的操作次数，如果对增广矩阵中的b(右侧向量)呢？因为b就一列，所以这里是$$n^2$$次

## lecture5 转置、置换和向量空间

接下来讨论可以进行行交换的场景，即主元位置存在0的情况，$$A=LU$$就变成了$$PA=LU$$，即对于任意可逆矩阵$$A$$，均有$$PA=LU$$

置换（permutation）：从单位矩阵出发，对其**行进行交换**，可以得到$$A_n^n=n!$$个置换，它们的逆就在这些置换矩阵之中(其实就是它的转置)，它们的转置也在这些置换矩阵之中，它们的两两的乘积也在置换矩阵之中，这就构成了一个矩阵乘法运算下的群。

性质：$$P^{-1}=P^T$$，然后就有$$P^TP=I$$

转置（transpose）：行列互换，即$$(A)^T_{ij}=A_{ji}$$

对称矩阵（symmetric）：转置后还是本身，即$$A^T=A$$

定理：对于任意一个矩阵$$R$$，$$R^TR$$是一个对称矩阵。

因为转置后的第i行=现在的第i列，转置后的第j行=现在的第j列，结果矩阵的第i行第j列=转置后的第i行和现在的第j列的内积，结果矩阵的第j行第i列=转置后的第j行和现在的第i列的内积=现在的第j列和转置后的第i行的内积，所以这两个相等，所以是对称矩阵。

证明：假设$$A=R^TR$$，$$(R^TR)^T=R^TR$$，即$$A^T=A$$
